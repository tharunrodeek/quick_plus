...
<div id="myModalLeaveVerify" class="modal fade" role="dialog">
    ...
    font-weight: bold;">Number of days requested</td><td>:</td><td>
                            <input type="text" id="lblLeaveReqDays" name="lblLeaveReqDays" disabled="disabled"/>
                        </td>
                    </tr>
                    <tr>
                        <td>Choose paid or unpaid </td><td>:</td>
                        <td>
                           <select id="ddlLeaveDeduction" class="form-control">
                               <option value="0">---Select---</option>
                               <option value="1">Paid leave</option>
                               <option value="2">Unpaid</option>
                               <option value="3">Halfday salary cut</option>
                           </select>
                        </td>
                    </tr>
                   <!-- <tr>
...
width: 104px;
    ...
    z-index: 9999;
  background: url('<?= $base_url ?>assets/media/img/ajax-loader_new.gif') 
              50% 50% ;
}
</style>
<script>
   ...
   function databind(type_id)
   {
    ...
    $('.ClsApporveReq').click(function()
    {
        ...
        var days_reduce_cmnt='';
        var leave_deduction_option=$("#ddlLeaveDeduction").val();
       ...
       if(leave_deduction_option!='0')
        {
            ...
            if(confrim==true) {
                ...
                AxisPro.APICall('POST', ERP_FUNCTION_API_END_POINT + "?method=approve_leave_request_with_split_date",
                    ...
                    +'&req_date='+days_req+'&days_reduce_cmnt='+days_reduce_cmnt
                    +'&salary_deduction_option='+leave_deduction_option, function (data) {
                        if (data.status == 'Error') {
                            toastr.error(data.msg);
                        }
                        else
                        {
                            toastr.success(data.msg);
                           // alert($('#hdn_changed_type_val').val());
                            databind($('#hdn_changed_type_val').val());
                        }
                    });
            }
        ...
        {
            toastr.error('Choose paid or unpaid or halfday');
        }
    });
