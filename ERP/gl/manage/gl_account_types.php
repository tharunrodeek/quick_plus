...
page(trans($help_context = "GL Account Groups"));
...
function can_process($selected_id) 
{
	...
	{
	    display_error( trans("The account group id cannot be empty."));
	    set_focus('id');
	    return false;
	}
	if (strlen(trim($_POST['name'])) == 0) 
	{
		display_error( trans("The account group name cannot be empty."));
		set_focus('name');
		return false;
	}
	...
	{
		display_error( trans("This account group id is already in use."));
		set_focus('id');
		return false;
	}
	...
	{
		display_error(trans("You cannot set an account group to be a subgroup of itself."));
		return false;
	}
	...
	if (can_process($selected_id)) 
	{
    	...
    	if ($selected_id != "") 
    	{
    		if (update_account_type($_POST['id'], $_POST['name'], $_POST['class_id'], $_POST['parent'], $_POST['old_id']))
				display_notification(trans('Selected account type has been updated'));
    	} 
    	else 
    	{
    		if (add_account_type($_POST['id'], $_POST['name'], $_POST['class_id'], $_POST['parent'])) {
				display_notification(trans('New account type has been added'));
			}
    	}
		$Mode = 'RESET';
	}
...
function can_delete($type)
{
	...
	{
		display_error(trans("Cannot delete this account group because GL accounts have been created referring to it."));
		return false;
	}

	if (key_in_foreign_table($type, 'chart_types', 'parent'))
	{
		display_error(trans("Cannot delete this account group because GL account groups have been created referring to it."));
		return false;
	}
...
if ($Mode == 'Delete')
{
	...
	if (can_delete($selected_id))
	{
		...
		display_notification(trans('Selected account group has been deleted'));
	}
	$Mode = 'RESET';
}
if ($Mode == 'RESET')
...
$th = array(trans("Group ID"), trans("Group Name"), trans("Subgroup Of"), trans("Class"), "", "");
...
text_row_ex(trans("ID:"), 'id', 10);
text_row_ex(trans("Name:"), 'name', 50);

gl_account_types_list_row(trans("Subgroup Of:"), 'parent', null, trans("None"), true);

if ($filter_cid)
	class_list_row(trans("Class:"), 'class_id', $_POST['cid']);
else
	class_list_row(trans("Class:"), 'class_id', null);
