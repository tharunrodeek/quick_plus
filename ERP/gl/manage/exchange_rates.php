...
page(trans($help_context = "Exchange Rates"), false, false, "", $js);
...
function check_data($selected_id)
{
	...
	{
		display_error( trans("The exchange rate cannot be zero or a negative number."));
		set_focus('BuyRate');
		return false;
	}
	if (!$selected_id && get_date_exchange_rate($_POST['curr_abrev'], $_POST['date_']))
	{
		display_error( trans("The exchange rate for the date is already there."));
		set_focus('date_');
		return false;
	}
	return true;
...
{
  return button('Edit'.$row["id"], trans("Edit"), true, ICON_EDIT);
}

function del_link($row) 
{
  return button('Delete'.$row["id"], trans("Delete"), true, ICON_DELETE);
}
...
function display_rate_edit()
{
	...
	if ($selected_id != "")
	{
		...
		label_row(trans("Date to Use From:"), $_POST['date_']);
	}
	else
	{
		$_POST['date_'] = Today();
		$_POST['BuyRate'] = '';
		date_row(trans("Date to Use From:"), 'date_');
	}
	...
	amount_row(trans("Exchange Rate:"), 'BuyRate', null, '',
	  	submit('get_rate',trans("Get"), false, trans('Get current rate from') . ' ' . $xchg_rate_provider , true), 'max');

	end_table(1);

	submit_add_or_update_center($selected_id == '', '', 'both');

	display_note(trans("Exchange rates are entered against the company currency."), 1);
}
...
$cols = array(
	trans("Date to Use From") => 'date', 
	trans("Exchange Rate") => 'rate',
...
if (is_company_currency(get_post('curr_abrev')))
{
	...
	display_note(trans("The selected currency is the company currency."), 2);
	display_note(trans("The company currency is the base currency so exchange rates cannot be set for it."), 1);
}
else
{
