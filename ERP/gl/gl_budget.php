...
page(trans($help_context = "Budget Entry"));
...
if (isset($_POST['add']) || isset($_POST['delete']))
{
	...
	if (isset($_POST['add']))
		display_notification_centered(trans("The Budget has been saved."));
	else
		display_notification_centered(trans("The Budget has been deleted."));
...
if (db_has_gl_accounts())
{
	...
	fiscalyears_list_row(trans("Fiscal Year:"), 'fyear', null);
	gl_all_accounts_list_row(trans("Account Code:"), 'account', null);
	if (!isset($_POST['dim1']))
		$_POST['dim1'] = 0;
	if (!isset($_POST['dim2']))
		$_POST['dim2'] = 0;
    if ($dim == 2)
    {
		dimensions_list_row(trans("Dimension")." 1", 'dim1', $_POST['dim1'], true, null, false, 1);
		dimensions_list_row(trans("Dimension")." 2", 'dim2', $_POST['dim2'], true, null, false, 2);
	}
	elseif ($dim == 1)
	{
		dimensions_list_row(trans("Dimension"), 'dim1', $_POST['dim1'], true, null, false, 1);
		hidden('dim2', 0);
	}
	else
	{
	...
	submit_row('submit', trans("Get"), true, '', '', true);
	end_table(1);
	...
	if ($showdims)
		$th = array(trans("Period"), trans("Amount"), trans("Dim. incl."), trans("Last Year"));
	else
		$th = array(trans("Period"), trans("Amount"), trans("Last Year"));
	table_header($th);
	$year = $_POST['fyear'];
	if (get_post('update') == '') {
	...
	submit('add', trans("Save"), true, '', 'default');
	submit_center_last('delete', trans("Delete"), '', true);
}
end_form();
