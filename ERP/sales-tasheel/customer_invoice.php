...
if (isset($_GET['AddedID'])) {
	...
	hyperlink_params("$path_to_root/sales-tasheel/inquiry/sales_deliveries_view.php", trans("Select Another &Delivery For Invoicing"), "OutstandingOnly=1");

	if (!db_num_rows(get_allocatable_from_cust_transactions(null, $invoice_no, $trans_type)))
		hyperlink_params("$path_to_root/sales-tasheel/customer_payments.php", trans("Entry &customer payment for this invoice"),
		"SInvoice=".$invoice_no);
	...
	hyperlink_no_params($path_to_root . "/sales-tasheel/inquiry/customer_inquiry.php", trans("Select Another &Invoice to Modify"));
...
if ( (isset($_GET['DeliveryNumber']) && ($_GET['DeliveryNumber'] > 0) )
	...
	if ($dn->count_items() == 0) {
		hyperlink_params($path_to_root . "/sales-tasheel/inquiry/sales_deliveries_view.php",
...
elseif (!processing_active()) {
	...
	hyperlink_no_params("$path_to_root/sales-tasheel/inquiry/sales_deliveries_view.php", trans("Select Delivery to Invoice"));
...
//    display_error(print_r($_SESSION['Items']->line_items ,true)); die;
...
//        display_error(print_r(24234 ,true)); die;
...
//            display_error(print_r("SELECT COUNT(*) FROM " . TB_PREF . "debtor_trans_details WHERE transaction_id = '$row->transaction_id' AND id != $row->id" ,true)); die;


            if(!empty($row->transaction_id)) {

                if (check_duplicates_row("SELECT COUNT(*) FROM " . TB_PREF . "debtor_trans_details WHERE transaction_id = '$row->transaction_id' AND id != $row->id")) {
...
//        display_error(print_r(check_data() ,true)); die;
...
if ($prepaid)
	...
	$th = array(trans("Item Code"), trans("Item Description"), trans("Quantity"), trans("Govt.Fee"),
		trans("Bank Charge (Inc. VAT)"), trans("Service Charge"), trans("Discount"),trans("Transaction ID"),
...
//    display_error(print_r($ln_itm ,true));
...
//		small_qty_cells(null, 'Line'.$line, qty_format($ln_itm->qty_dispatched, $ln_itm->stock_id, $dec), null, null, $dec);
    ...
    text_cells(null, 'Line'.$line.'application_id', $ln_itm->application_id, 10, 50);
    text_cells(null, 'Line'.$line.'ref_name', $ln_itm->ref_name, 10, 50);
