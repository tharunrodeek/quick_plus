...
display_heading(sprintf($myrow['prep_amount'] > 0 ? (
	$paym['days_before_due']>=0 ? trans("FINAL INVOICE #%d") : trans("PREPAYMENT INVOICE #%d")) : trans("SALES INVOICE #%d"),$trans_id));
...
if (db_num_rows($result) > 0)
{
	...
	while ($myrow2 = db_fetch($result))
	{
		...
		$value = round2(((1 - $myrow2["discount_percent"]) * $myrow2["unit_price"] * $myrow2["quantity"])+$govt_charges,
		   user_price_dec());
		$sub_total += $value;
