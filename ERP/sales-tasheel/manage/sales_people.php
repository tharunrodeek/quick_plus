...
page(trans($help_context = "Sales Persons"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(trans("The sales person name cannot be empty."));
		set_focus('salesman_name');
	}
	$pr1 = check_num('provision', 0,100);
	if (!$pr1 || !check_num('provision2', 0, 100)) {
		$input_error = 1;
		display_error( trans("Salesman provision cannot be less than 0 or more than 100%."));
		set_focus(!$pr1 ? 'provision' : 'provision2');
	}
	if (!check_num('break_pt', 0)) {
		$input_error = 1;
		display_error( trans("Salesman provision breakpoint must be numeric and not less than 0."));
		set_focus('break_pt');
	}
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1) 
			display_notification(trans('Selected sales person data have been updated'));
		else
			display_notification(trans('New sales person data have been added'));
		$Mode = 'RESET';
	}
}
if ($Mode == 'Delete')
{
	...
	{
		display_error(trans("Cannot delete this sales-person because branches are set up referring to this sales-person - first alter the branches concerned."));
	}
	else
	{
		delete_salesman($selected_id);
		display_notification(trans('Selected sales person data have been deleted'));
	}
	$Mode = 'RESET';
}
...
$th = array(trans("Name"), trans("Phone"), trans("Fax"), trans("Email"), trans("Provision"), trans("Break Pt."), trans("Provision")." 2", "", "");
...
while ($myrow = db_fetch($result))
{
	...
	inactive_control_cell($myrow["salesman_code"], $myrow["inactive"],
 	...
 	edit_button_cell("Edit".$myrow["salesman_code"], trans("Edit"));
 	delete_button_cell("Delete".$myrow["salesman_code"], trans("Delete"));
  	end_row();
...
text_row_ex(trans("Sales person name:"), 'salesman_name', 30);
text_row_ex(trans("Telephone number:"), 'salesman_phone', 20);
text_row_ex(trans("Fax number:"), 'salesman_fax', 20);
email_row_ex(trans("E-mail:"), 'salesman_email', 40);
percent_row(trans("Provision").':', 'provision');
amount_row(trans("Turnover Break Pt Level:"), 'break_pt');
percent_row(trans("Provision")." 2:", 'provision2');
end_table(1);
