...
function dispatch_link($row)
{
	...
	if ($trans_type == ST_SALESORDER)
  		return pager_link( trans("Dispatch"),
			"/sales-tasheel/customer_delivery.php?OrderNumber=" .$row['order_no'], ICON_DOC);
	else
  		return pager_link( trans("Sales Order"),
			"/sales-tasheel/sales_order_entry.php?OrderNumber=" .$row['order_no'], ICON_DOC);
}
...
function invoice_link($row)
{
	...
	if ($trans_type == ST_SALESORDER)
  		return pager_link( trans("Invoice"),
			"/sales-tasheel/sales_order_entry.php?NewInvoice=" .$row["order_no"], ICON_DOC);
	else
		return '';
}
...
function delivery_link($row)
{
  return pager_link( trans("Delivery"),
	"/sales-tasheel/sales_order_entry.php?NewDelivery=" .$row['order_no'], ICON_DOC);
}

function order_link($row)
{
  return pager_link( trans("Sales Order"),
	"/sales-tasheel/sales_order_entry.php?NewQuoteToSalesOrder=" .$row['order_no'], ICON_DOC);
}
...
function invoice_prep_link($row)
{
	...
	return 
		...
		"/sales-tasheel/customer_invoice.php?InvoicePrepayments=" .$row['order_no'], ICON_DOC) : '';
}
...
if ($trans_type == ST_SALESORDER)
	$cols = array(
		trans("Order #") => array('fun'=>'view_link', 'align'=>'right'),
		trans("Ref") => array('type' => 'sorder.reference', 'ord' => '') ,
		trans("Customer") => array('type' => 'debtor.name' , 'ord' => '') ,
		trans("Branch"),
		trans("Cust Order Ref"),
		trans("Order Date") => array('type' =>  'date', 'ord' => ''),
		trans("Required By") =>array('type'=>'date', 'ord'=>''),
		trans("Delivery To"),
		trans("Order Total") => array('type'=>'amount', 'ord'=>''),
		'Type' => 'skip',
		trans("Currency") => array('align'=>'center')
	);
else
	$cols = array(
		trans("Quote #") => array('fun'=>'view_link', 'align'=>'right'),
		trans("Ref"),
		trans("Customer"),
		trans("Branch"),
		trans("Cust Order Ref"),
		trans("Quote Date") => 'date',
		trans("Valid until") =>array('type'=>'date', 'ord'=>''),
		trans("Delivery To"),
		trans("Quote Total") => array('type'=>'amount', 'ord'=>''),
		'Type' => 'skip',
		trans("Currency") => array('align'=>'center')
	);
