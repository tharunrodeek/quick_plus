...
function print_sales_summary_report()
{
	...
	$rep = new FrontReport(trans('Sales Summary Report'), "SalesSummaryReport", user_pagesize(), 9, $orientation);

	$params =   array( 	0 => $comments,
						1 => array('text' => trans('Period'), 'from' => $from, 'to' => $to),
						2 => array(  'text' => trans('Tax Id Only'),'from' => $tid,'to' => ''));

	$cols = array(0, 130, 180, 270, 350, 500);

	$headers = array(trans('Customer'), trans('Tax Id'), trans('Total ex. Tax'), trans('Tax'));
	...
	$rep->TextCol(0, 4, trans('Balances in Home Currency'));
	$rep->NewLine(2);
