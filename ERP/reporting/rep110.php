...
function print_deliveries()
{
	...
	{
		if ($packing_slip == 0)
			$rep = new FrontReport(trans('DELIVERY'), "DeliveryNoteBulk", user_pagesize(), 9, $orientation);
		else
			$rep = new FrontReport(trans('PACKING SLIP'), "PackingSlipBulk", user_pagesize(), 9, $orientation);
	}
    if ($orientation == 'L')
	...
	for ($i = $from; $i <= $to; $i++)
	{
			...
			if ($email == 1)
			{
				...
				if ($packing_slip == 0)
				{
					$rep->title = trans('DELIVERY NOTE');
					$rep->filename = "Delivery" . $myrow['reference'] . ".pdf";
				}
				else
				{
					$rep->title = trans('PACKING SLIP');
			...
			if ($packing_slip == 0)
			{
				...
				$DisplayTotal = number_format2($myrow["ov_freight"] +$myrow["ov_freight_tax"] + $myrow["ov_gst"] +
					$myrow["ov_amount"],$dec);
				$rep->Font('bold');
				$rep->TextCol(3, 6, trans("TOTAL DELIVERY INCL. VAT"), - 2);
