...
function print_outstanding_GRN()
{
	...
	$headers = array(trans('GRN'), trans('Order'), trans('Item') . '/' . trans('Description'), trans('Qty Recd'), trans('qty Inv'), trans('Balance'),
		trans('Act Price'), trans('Value'));

	$aligns = array('left',	'left',	'left',	'right', 'right', 'right', 'right', 'right');

    $params =   array( 	0 => $comments,
    				    1 => array('text' => trans('Supplier'), 'from' => $from, 'to' => ''));

    $rep = new FrontReport(trans('Outstanding GRNs Report'), "OutstandingGRN", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
