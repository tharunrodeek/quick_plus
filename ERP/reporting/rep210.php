...
//----------------------------------------------------------------------------------------------------
    function get_remittance($type, $trans_no,$voided=false)
    {
    	if ($voided){
    		$sql = "SELECT trans.*, 
    		...
    		supplier.address
    		FROM "
    		.TB_PREF."voided_supp_trans trans,"
    ...
    function print_remittances()
    {
    	...
    	$to = max($fno[0], $tno[0]);
    	$voided = get_voided_entry($fno[1], $tno[0]) == null ? false : true;
	...
	// $headers in doctext.inc
    	...
    	if ($email == 0)
    		$rep = new FrontReport($voided ? trans("VOIDED REMITTANCE") : trans("REMITTANCE"), "RemittanceBulk", user_pagesize(), 9, $orientation);
    	if ($orientation == 'L')
    	...
    	for ($i = $from; $i <= $to; $i++)
    	{
    		...
    		foreach ($types as $j)
    		{
    			$void_entry = get_voided_entry($j, $i);
    			$myrow = get_remittance($j, $i,$voided = $void_entry == null ? false : true);
    			if (!$myrow)
    				continue;
    			...
    			$rep->NewPage();
    			$result = get_allocatable_to_supp_transactions($myrow['supplier_id'], $myrow['trans_no'], $myrow['type'],$voided);
    			// echo "<pre>";
    			// print_r($result);
    			// exit;
			// exit;
    			...
    			$rep->TextCol(3, 6, trans("Total Allocated"), -2);
    			$rep->AmountCol(6, 7, $total_allocated, $dec, -2);
    			$rep->NewLine();
    			$rep->TextCol(3, 6, trans("Left to Allocate"), -2);
    			$myrow['Total'] *= -1;
    			$myrow['ov_discount'] *= -1;
    			...
    			$rep->NewLine();
    			$rep->Font('bold');
    			$rep->TextCol(3, 6, trans("TOTAL REMITTANCE"), - 2);
