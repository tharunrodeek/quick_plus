...
function print_supplier_balances()
{
	...
	$cols = array(0, 50, 110, 180,	240, 300, 360, 415,	515);

	$headers = array(_('Type'), _('#'), _('Date'), _('Due Date'), _('Charges'),
		_('Credits'), _('Allocated'), _('Balance'));
...
//				4 => array('text' => _('Suppress Zeros'), 'from' => $nozeros, 'to' => '')
    ...
    $rep = new FrontReport(_('Supplier Statement'), "SupplierBalances", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
...
//		$rep->Line($rep->row + 4);
		...
		while ($trans=db_fetch($res))
		{
			...
			$rep->NewLine(1, 2);
			$rep->TextCol(0, 1, ltrim($systypes_array[$trans['type']],'Supplier'));
