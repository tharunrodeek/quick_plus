...
function print_order_status_list()
{
	...
	if ($backorder == 0)
		$back = trans('All Orders');
	else
		$back = trans('Back Orders Only');

	$cols = array(0, 60, 150, 260, 325,	385, 450, 515);

	$headers2 = array(trans('Order'), trans('Customer'), trans('Branch'), trans('Customer Ref'),
		trans('Ord Date'),	trans('Del Date'),	trans('Loc'));

	$aligns = array('left',	'left',	'right', 'right', 'right', 'right',	'right');

	$headers = array(trans('Code'),	trans('Description'), trans('Ordered'),	trans('Delivered'),
		trans('Outstanding'), '', trans('Total Amount'));
    ...
    $params =   array( 	0 => $comments,
	    				...
	    				4 => array(  'text' => trans('Selection'),'from' => $back,'to' => ''));

	$aligns2 = $aligns;

	$rep = new FrontReport(trans('Order Status Listing'), "OrderStatusListing", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	while ($myrow=db_fetch($result))
	{
		...
		{
			$rep->Font('italic');
			$rep->TextCol(5, 6,	trans('Outstanding'));
			$rep->Font();
		}
		$rep->NewLine();
	}
	$rep->Line($rep->row);
	$rep->NewLine();
	$rep->TextCol(1, 6, trans("Grand Total"));
