...
function print_bank_transactions_reconcile()
{
	...
	$rep = new FrontReport(trans('Bank Statement w/Reconcile'), "BankStatementReconcile", user_pagesize(), 9, "L");
	$dec = user_price_dec();
	...
	$headers = array(trans('Type'),	trans('#'),	trans('Reference'), trans('Date'), trans('Person/Item'),
		trans('Debit'),	trans('Credit'), trans('Balance'), trans('Reco Date'), trans('Narration'));
	...
	if ($prev_balance != 0.0 || $rows != 0)
	{
		...
		$rep->Font('bold');
		$rep->TextCol(3, 5,	trans("Bank Balance"));
		if ($reconciled > 0.0)
		...
		// Reco Difference
		$rep->Font('bold');
		$rep->TextCol(3, 5,	trans("Difference"));
		if ($difference > 0.0)
