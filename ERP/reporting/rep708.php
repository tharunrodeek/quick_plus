...
function display_type ($type, $typename, &$dec, &$rep, $from, $to, $zero, $balances, $dimension, $dimension2)
{
	...
	while ($account=db_fetch($accounts))
	{
		...
		{	
			$rep->row -= 4;
			$rep->TextCol(0, 8, trans("Group")." - ".$type ." - ".$typename);
			$printtitle = 1;
			$rep->row -= 4;
			$rep->Line($rep->row);
			$rep->NewLine();			
...
function print_trial_balance()
{
	...
	$headers2 = array('', '', trans('Brought Forward'),	trans('This Period'), trans('Balance'));
	...
	$headers = array(trans('Account'), trans('Account Name'), trans('Debit'), trans('Credit'), trans('Debit'),
		trans('Credit'), trans('Debit'), trans('Credit'));
	...
	$rep = new FrontReport(trans('Trial Balance'), "TrialBalance", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
    {
	...
	$rep->TextCol(0, 2, trans("Ending Balance"));
	...
	if (($pbal = round2($pbal, $dec)) != 0.0 && $dimension == 0 && $dimension2 == 0)
	{
		$rep->NewLine(2);
		$rep->Font();
		$rep->TextCol(0, 8, trans("The Opening Balance is not in balance, probably due to a non closed Previous Fiscalyear."));
	}	
	$rep->End();
}
