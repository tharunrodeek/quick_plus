...
function print_supplier_details_listing()
{
	...
	$headers = array(trans('Mailing Address:'), trans('Turnover'),	trans('Contact Information'),
    ...
    $params =   array( 	0 => $comments,
    				    ...
    				    2 => array('text' => trans('Activity'), 		'from' => $morestr, 	'to' => $lessstr . " " . get_company_pref("curr_default")));

    $rep = new FrontReport(trans('Supplier Details Listing'), "SupplierDetailsListing", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	while ($myrow=db_fetch($result))
	{
		...
		if ($printsupplier)
		{
			...
			$rep->TextCol(1, 2,	trans('Tax_Id') . ": " . $myrow['gst_no']);
