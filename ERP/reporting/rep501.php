...
function print_dimension_summary()
{
	...
	$headers = array(trans('Reference'), trans('Name'), trans('Type'), trans('Date'), trans('Due Date'), trans('Closed'), trans('YTD'));

	$aligns = array('left',	'left', 'left',	'left', 'left', 'left', 'right');

    $params =   array( 	0 => $comments,
    				    1 => array('text' => trans('Dimension'), 'from' => get_dimension_string($fromdim), 'to' => get_dimension_string($todim)));

    $rep = new FrontReport(trans('Dimension Summary'), "DimensionSummary", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
