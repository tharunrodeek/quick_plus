...
page(trans($help_context = "View Work Order"), true, false, "", $js);
...
{
    display_heading2(trans("BOM for item:") . " " . $myrow["StockItemName"]);
    display_bom($myrow["stock_id"]);
}
else
{
	display_heading2(trans("Work Order Requirements"));
	display_wo_requirements($woid, $myrow["units_reqd"]);
	if ($myrow["type"] == WO_ADVANCED)
	{
    	echo "<br><table cellspacing=7><tr valign=top><td>";
    	display_heading2(trans("Issues"));
    	display_wo_issues($woid);
    	echo "</td><td>";
    	display_heading2(trans("Productions"));
    	display_wo_productions($woid);
    	echo "</td><td>";
    	display_heading2(trans("Additional Costs"));
...
is_voided_display(ST_WORKORDER, $woid, trans("This work order has been voided."));
