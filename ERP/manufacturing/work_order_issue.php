...
page(trans($help_context = "Issue Items to Work Order"), false, false, "", $js);
...
{
	$id = $_GET['AddedID'];
   	display_notification(trans("The work order issue has been entered."));

    display_note(get_trans_view_str(ST_WORKORDER, $id, trans("View this Work Order")));

   	display_note(get_gl_view_str(ST_WORKORDER, $id, trans("View the GL Journal Entries for this Work Order")), 1);
...
function can_process()
{
	...
	{
		display_error(trans("The entered date for the issue is invalid."));
		set_focus('date_');
		return false;
	} 
	...
	if ($failed_item)
	{
   		display_error(trans("The issue cannot be processed because it would cause negative inventory balance for marked items as of document date or later."));
		return false;
	}
...
if (isset($_POST['Process']) && can_process())
{
	...
	if ($failed_data != null) 
	{
		display_error(trans("The process cannot be completed because there is an insufficient total quantity for a component.") . "<br>"
		. trans("Component is :"). $failed_data[0] . "<br>"
		. trans("From location :"). $failed_data[1] . "<br>");
	} 
	else 
	{
...
echo "<tr><td>";
display_issue_items(trans("Items to Issue"), $_SESSION['issue_items']);
issue_options_controls();
echo "</td></tr>";

end_table();

submit_center('Process', trans("Process Issue"), true, '', 'default');
