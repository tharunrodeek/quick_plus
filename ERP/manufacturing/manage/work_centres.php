...
page(trans($help_context = "Work Centres"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(trans("The work centre name cannot be empty."));
		set_focus('name');
	}
	...
	if ($input_error != 1) 
	{
    	...
    	if ($selected_id != -1) 
    	{
			...
			display_notification(trans('Selected work center has been updated'));
    	} 
    	else 
    	{
    		add_work_centre($_POST['name'], $_POST['description']);
			display_notification(trans('New work center has been added'));
    	}
		$Mode = 'RESET';
	}
} 
...
function can_delete($selected_id)
{
	...
	{
		display_error(trans("Cannot delete this work centre because BOMs have been created referring to it."));
		return false;
	}

	if (key_in_foreign_table($selected_id, 'wo_requirements', 'workcentre'))	
	{
		display_error(trans("Cannot delete this work centre because work order requirements have been created referring to it."));
		return false;
	}		
...
if ($Mode == 'Delete')
{
	...
	if (can_delete($selected_id))
	{
		...
		display_notification(trans('Selected work center has been deleted'));
	}
	$Mode = 'RESET';
}
...
$th = array(trans("Name"), trans("description"), "", "");
...
text_row_ex(trans("Name:"), 'name', 40);
