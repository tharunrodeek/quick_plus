...
class EmployeeCommissionReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if(!empty($request->All())) {
                ...
                $result_object = $this->arrayPaginator($request->All(),new EmployeeCommissionReport());
        ...
        return view('employee_commission_report/list', compact(
    ...
    public function export(Request $request, $type = 'pdf')
    {
        ...
        try {
            ...
            $data = $this->getReport($request->All(),new EmployeeCommissionReport());
            Excel::create('employee_commission_report', function ($excel) use ($data) {

                $excel->setTitle('Employee Comm. Report');
                $excel->setDescription('Employee Comm. Report');
                $excel->sheet('AxisPro-EmployeeCommReport', function ($sheet) use ($data) {
                    ...
                    if (!empty($data)) {
                        ...
                        foreach ($data as $key => $row) {
                            ...
                            $sheet->cell('G' . $i, $row->user_commission);
                            $sheet->cell('H' . $i, $row->quantity);
                            $sheet->cell('I' . $i, $row->total_commission);
            ...
            $fileName = storage_path()."/exports/employee_commission_report.$type";
