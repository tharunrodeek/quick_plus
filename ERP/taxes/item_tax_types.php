...
page(trans($help_context = "Item Tax Types"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(trans("The item tax type description cannot be empty."));
		set_focus('name');
	}
	...
	if ($input_error != 1) 
	{
    	...
    	{    		
			...
			display_notification(trans('Selected item tax type has been updated'));
    	} 
    	else 
    	{
    		add_item_tax_type($_POST['name'], $_POST['exempt'], $exempt_from);
			display_notification(trans('New item tax type has been added'));
    	}
		$Mode = 'RESET';
	}
} 
...
function can_delete($selected_id)
{
	...
	{
		display_error(trans("Cannot delete this item tax type because items have been created referring to it."));
		return false;
	}
	if (key_in_foreign_table($selected_id, 'stock_category', 'dflt_tax_type'))
	{
		display_error(trans("Cannot delete this item tax type because item categories have been created referring to it."));
		return false;
	}
...
if ($Mode == 'Delete')
{
	...
	if (can_delete($selected_id))
	{
		...
		display_notification(trans('Selected item tax type has been deleted'));
	}
	$Mode = 'RESET';
}
...
$th = array(trans("Name"), trans("Tax exempt"),'','');
...
while ($myrow = db_fetch($result2)) 
{
	...
	if ($myrow["exempt"] == 0) 
	{
		$disallow_text = trans("No");
	} 
	else 
	{
		$disallow_text = trans("Yes");
	}
...
yesno_list_row(trans("Is Fully Tax-exempt:"), 'exempt', null, "", "", true);
...
if (!isset($_POST['exempt']) || $_POST['exempt'] == 0) 
{
    ...
    display_note(trans("Select which taxes this item tax type is exempt from."), 0, 1);
    
    start_table(TABLESTYLE2);
    $th = array(trans("Tax Name"), trans("Rate"), trans("Is exempt"));
    table_header($th);
