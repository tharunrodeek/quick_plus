...
page(trans($help_context = "Suppliers"), false, false, "", $js);
...
function supplier_settings(&$supplier_id)
{
	...
	text_row(trans("Supplier Name:"), 'supp_name', null, 42, 40);
	text_row(trans("Supplier Short Name:"), 'supp_ref', null, 30, 30);

	text_row(trans("GSTNo:"), 'gst_no', null, 42, 40);
	link_row(trans("Website:"), 'website', null, 35, 55);
	...
	if ($supplier_id) 
	{
		submit_center_first('submit', trans("Update Supplier"),
		  trans('Update supplier data'), $page_nested ? true : 'default');
	...
	{
		submit_center('submit', trans("Add New Supplier Details"), true, '', 'default');
	}
	div_end();
}
...
if (db_has_suppliers()) 
{
	...
	supplier_list_cells(trans("Select a supplier: "), 'supplier_id', null,
		  trans('New supplier'), true, check_value('show_inactive'));
