...
<?php include_once "invoice_report_viewinfo.php" ?>
...
$invoice_report_view_list = NULL; // Initialize page object first

class cinvoice_report_view_list extends cinvoice_report_view {
	...
	// Table name
	var $TableName = 'invoice_report_view';

	// Page object name
	var $PageObjName = 'invoice_report_view_list';

	// Grid form hidden field names
	var $FormName = 'finvoice_report_viewlist';
	...
	function __construct() {
		...
		// Table object (invoice_report_view)
		if (!isset($GLOBALS["invoice_report_view"]) || get_class($GLOBALS["invoice_report_view"]) == "cinvoice_report_view") {
			$GLOBALS["invoice_report_view"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["invoice_report_view"];
		}
		...
		$this->AddUrl = "invoice_report_viewadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "invoice_report_viewdelete.php";
		$this->MultiUpdateUrl = "invoice_report_viewupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'invoice_report_view', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption finvoice_report_viewlistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $invoice_report_view;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($invoice_report_view);
				$doc->Text = $sContent;
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "finvoice_report_viewlistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"finvoice_report_viewlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"finvoice_report_viewlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.finvoice_report_viewlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"finvoice_report_viewlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_invoice_report_view\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_invoice_report_view',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.finvoice_report_viewlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($invoice_report_view_list)) $invoice_report_view_list = new cinvoice_report_view_list();

// Page init
$invoice_report_view_list->Page_Init();

// Page main
$invoice_report_view_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$invoice_report_view_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($invoice_report_view->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = finvoice_report_viewlist = new ew_Form("finvoice_report_viewlist", "list");
finvoice_report_viewlist.FormKeyCountName = '<?php echo $invoice_report_view_list->FormKeyCountName ?>';

// Form_CustomValidate event
finvoice_report_viewlist.Form_CustomValidate = 
...
finvoice_report_viewlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
finvoice_report_viewlist.Lists["x_customer_name[]"] = {"LinkField":"x_name","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_debtors_master"};
finvoice_report_viewlist.Lists["x_customer_name[]"].Data = "<?php echo $invoice_report_view_list->customer_name->LookupFilterQuery(FALSE, "list") ?>";
finvoice_report_viewlist.Lists["x_created_employee[]"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_id","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_users"};
finvoice_report_viewlist.Lists["x_created_employee[]"].Data = "<?php echo $invoice_report_view_list->created_employee->LookupFilterQuery(FALSE, "list") ?>";
finvoice_report_viewlist.Lists["x_payment_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finvoice_report_viewlist.Lists["x_payment_status"].Options = <?php echo json_encode($invoice_report_view_list->payment_status->Options()) ?>;

// Form object for search
var CurrentSearchForm = finvoice_report_viewlistsrch = new ew_Form("finvoice_report_viewlistsrch");

// Validate function for search
finvoice_report_viewlistsrch.Validate = function(fobj) {
	...
	if (elm && !ew_CheckDateDef(elm.value))
		return this.OnError(elm, "<?php echo ew_JsEncode2($invoice_report_view->transaction_date->FldErrMsg()) ?>");
...
// Form_CustomValidate event
finvoice_report_viewlistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
finvoice_report_viewlistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
finvoice_report_viewlistsrch.Lists["x_customer_name[]"] = {"LinkField":"x_name","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_debtors_master"};
finvoice_report_viewlistsrch.Lists["x_customer_name[]"].Data = "<?php echo $invoice_report_view_list->customer_name->LookupFilterQuery(FALSE, "extbs") ?>";
finvoice_report_viewlistsrch.Lists["x_created_employee[]"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_id","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_users"};
finvoice_report_viewlistsrch.Lists["x_created_employee[]"].Data = "<?php echo $invoice_report_view_list->created_employee->LookupFilterQuery(FALSE, "extbs") ?>";
finvoice_report_viewlistsrch.Lists["x_payment_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finvoice_report_viewlistsrch.Lists["x_payment_status"].Options = <?php echo json_encode($invoice_report_view_list->payment_status->Options()) ?>;
</script>
<script type="text/javascript">
...
<?php if ($invoice_report_view->Export == "") { ?>
<div class="ewToolbar">
<?php if ($invoice_report_view_list->TotalRecs > 0 && $invoice_report_view_list->ExportOptions->Visible()) { ?>
<?php $invoice_report_view_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($invoice_report_view_list->SearchOptions->Visible()) { ?>
<?php $invoice_report_view_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($invoice_report_view_list->FilterOptions->Visible()) { ?>
<?php $invoice_report_view_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $invoice_report_view_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($invoice_report_view_list->TotalRecs <= 0)
			$invoice_report_view_list->TotalRecs = $invoice_report_view->ListRecordCount();
	} else {
		if (!$invoice_report_view_list->Recordset && ($invoice_report_view_list->Recordset = $invoice_report_view_list->LoadRecordset()))
			$invoice_report_view_list->TotalRecs = $invoice_report_view_list->Recordset->RecordCount();
	}
	$invoice_report_view_list->StartRec = 1;
	if ($invoice_report_view_list->DisplayRecs <= 0 || ($invoice_report_view->Export <> "" && $invoice_report_view->ExportAll)) // Display all records
		$invoice_report_view_list->DisplayRecs = $invoice_report_view_list->TotalRecs;
	if (!($invoice_report_view->Export <> "" && $invoice_report_view->ExportAll))
		$invoice_report_view_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$invoice_report_view_list->Recordset = $invoice_report_view_list->LoadRecordset($invoice_report_view_list->StartRec-1, $invoice_report_view_list->DisplayRecs);

	// Set no record found message
	if ($invoice_report_view->CurrentAction == "" && $invoice_report_view_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$invoice_report_view_list->setWarningMessage(ew_DeniedMsg());
		if ($invoice_report_view_list->SearchWhere == "0=101")
			$invoice_report_view_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$invoice_report_view_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$invoice_report_view_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($invoice_report_view->Export == "" && $invoice_report_view->CurrentAction == "") { ?>
<form name="finvoice_report_viewlistsrch" id="finvoice_report_viewlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($invoice_report_view_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="finvoice_report_viewlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="invoice_report_view">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$invoice_report_view_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$invoice_report_view->RowType = EW_ROWTYPE_SEARCH;

// Render row
$invoice_report_view->ResetAttrs();
$invoice_report_view_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($invoice_report_view->invoice_no->Visible) { // invoice_no ?>
	<div id="xsc_invoice_no" class="ewCell form-group">
		<label for="x_invoice_no" class="ewSearchCaption ewLabel"><?php echo $invoice_report_view->invoice_no->FldCaption() ?></label>
...
<input type="text" data-table="invoice_report_view" data-field="x_invoice_no" name="x_invoice_no" id="x_invoice_no" size="30" maxlength="60" placeholder="<?php echo ew_HtmlEncode($invoice_report_view->invoice_no->getPlaceHolder()) ?>" value="<?php echo $invoice_report_view->invoice_no->EditValue ?>"<?php echo $invoice_report_view->invoice_no->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_2" class="ewRow">
<?php if ($invoice_report_view->transaction_date->Visible) { // transaction_date ?>
	<div id="xsc_transaction_date" class="ewCell form-group">
		<label for="x_transaction_date" class="ewSearchCaption ewLabel"><?php echo $invoice_report_view->transaction_date->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<input type="text" data-table="invoice_report_view" data-field="x_transaction_date" data-format="2" name="x_transaction_date" id="x_transaction_date" placeholder="<?php echo ew_HtmlEncode($invoice_report_view->transaction_date->getPlaceHolder()) ?>" value="<?php echo $invoice_report_view->transaction_date->EditValue ?>"<?php echo $invoice_report_view->transaction_date->EditAttributes() ?>>
<?php if (!$invoice_report_view->transaction_date->ReadOnly && !$invoice_report_view->transaction_date->Disabled && !isset($invoice_report_view->transaction_date->EditAttrs["readonly"]) && !isset($invoice_report_view->transaction_date->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("finvoice_report_viewlistsrch", "x_transaction_date", {"ignoreReadonly":true,"useCurrent":false,"format":2});
</script>
<?php } ?>
</span>
...
<input type="text" data-table="invoice_report_view" data-field="x_transaction_date" data-format="2" name="y_transaction_date" id="y_transaction_date" placeholder="<?php echo ew_HtmlEncode($invoice_report_view->transaction_date->getPlaceHolder()) ?>" value="<?php echo $invoice_report_view->transaction_date->EditValue2 ?>"<?php echo $invoice_report_view->transaction_date->EditAttributes() ?>>
<?php if (!$invoice_report_view->transaction_date->ReadOnly && !$invoice_report_view->transaction_date->Disabled && !isset($invoice_report_view->transaction_date->EditAttrs["readonly"]) && !isset($invoice_report_view->transaction_date->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("finvoice_report_viewlistsrch", "y_transaction_date", {"ignoreReadonly":true,"useCurrent":false,"format":2});
</script>
<?php } ?>
</span>
	</div>
...
<?php if ($invoice_report_view->customer_name->Visible) { // customer_name ?>
	<div id="xsc_customer_name" class="ewCell form-group">
		<label for="x_customer_name" class="ewSearchCaption ewLabel"><?php echo $invoice_report_view->customer_name->FldCaption() ?></label>
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_customer_name"><?php echo (strval($invoice_report_view->customer_name->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $invoice_report_view->customer_name->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($invoice_report_view->customer_name->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_customer_name[]',m:1,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($invoice_report_view->customer_name->ReadOnly || $invoice_report_view->customer_name->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="invoice_report_view" data-field="x_customer_name" data-multiple="1" data-lookup="1" data-value-separator="<?php echo $invoice_report_view->customer_name->DisplayValueSeparatorAttribute() ?>" name="x_customer_name[]" id="x_customer_name[]" value="<?php echo $invoice_report_view->customer_name->AdvancedSearch->SearchValue ?>"<?php echo $invoice_report_view->customer_name->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_4" class="ewRow">
<?php if ($invoice_report_view->created_employee->Visible) { // created_employee ?>
	<div id="xsc_created_employee" class="ewCell form-group">
		<label for="x_created_employee" class="ewSearchCaption ewLabel"><?php echo $invoice_report_view->created_employee->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_created_employee"><?php echo (strval($invoice_report_view->created_employee->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $invoice_report_view->created_employee->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($invoice_report_view->created_employee->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_created_employee[]',m:1,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($invoice_report_view->created_employee->ReadOnly || $invoice_report_view->created_employee->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="invoice_report_view" data-field="x_created_employee" data-multiple="1" data-lookup="1" data-value-separator="<?php echo $invoice_report_view->created_employee->DisplayValueSeparatorAttribute() ?>" name="x_created_employee[]" id="x_created_employee[]" value="<?php echo $invoice_report_view->created_employee->AdvancedSearch->SearchValue ?>"<?php echo $invoice_report_view->created_employee->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_5" class="ewRow">
<?php if ($invoice_report_view->payment_status->Visible) { // payment_status ?>
	<div id="xsc_payment_status" class="ewCell form-group">
		<label for="x_payment_status" class="ewSearchCaption ewLabel"><?php echo $invoice_report_view->payment_status->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_payment_status" id="z_payment_status" value="="></span>
		<span class="ewSearchField">
<select data-table="invoice_report_view" data-field="x_payment_status" data-value-separator="<?php echo $invoice_report_view->payment_status->DisplayValueSeparatorAttribute() ?>" id="x_payment_status" name="x_payment_status"<?php echo $invoice_report_view->payment_status->EditAttributes() ?>>
<?php echo $invoice_report_view->payment_status->SelectOptionListHtml("x_payment_status") ?>
</select>
</span>
	</div>
<?php } ?>
...
<div id="xsr_6" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($invoice_report_view_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($invoice_report_view_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $invoice_report_view_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($invoice_report_view_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($invoice_report_view_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($invoice_report_view_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($invoice_report_view_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $invoice_report_view_list->ShowPageHeader(); ?>
<?php
$invoice_report_view_list->ShowMessage();
?>
<?php if ($invoice_report_view_list->TotalRecs > 0 || $invoice_report_view->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($invoice_report_view_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> invoice_report_view">
<form name="finvoice_report_viewlist" id="finvoice_report_viewlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($invoice_report_view_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $invoice_report_view_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="invoice_report_view">
<div id="gmp_invoice_report_view" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($invoice_report_view_list->TotalRecs > 0 || $invoice_report_view->CurrentAction == "gridedit") { ?>
<table id="tbl_invoice_report_viewlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$invoice_report_view_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$invoice_report_view_list->RenderListOptions();

// Render list options (header, left)
$invoice_report_view_list->ListOptions->Render("header", "left");
?>
<?php if ($invoice_report_view->invoice_no->Visible) { // invoice_no ?>
	<?php if ($invoice_report_view->SortUrl($invoice_report_view->invoice_no) == "") { ?>
		<th data-name="invoice_no" class="<?php echo $invoice_report_view->invoice_no->HeaderCellClass() ?>"><div id="elh_invoice_report_view_invoice_no" class="invoice_report_view_invoice_no"><div class="ewTableHeaderCaption"><?php echo $invoice_report_view->invoice_no->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="invoice_no" class="<?php echo $invoice_report_view->invoice_no->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $invoice_report_view->SortUrl($invoice_report_view->invoice_no) ?>',1);"><div id="elh_invoice_report_view_invoice_no" class="invoice_report_view_invoice_no">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $invoice_report_view->invoice_no->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($invoice_report_view->invoice_no->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($invoice_report_view->invoice_no->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($invoice_report_view->transaction_date->Visible) { // transaction_date ?>
	<?php if ($invoice_report_view->SortUrl($invoice_report_view->transaction_date) == "") { ?>
		<th data-name="transaction_date" class="<?php echo $invoice_report_view->transaction_date->HeaderCellClass() ?>"><div id="elh_invoice_report_view_transaction_date" class="invoice_report_view_transaction_date"><div class="ewTableHeaderCaption"><?php echo $invoice_report_view->transaction_date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="transaction_date" class="<?php echo $invoice_report_view->transaction_date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $invoice_report_view->SortUrl($invoice_report_view->transaction_date) ?>',1);"><div id="elh_invoice_report_view_transaction_date" class="invoice_report_view_transaction_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $invoice_report_view->transaction_date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($invoice_report_view->transaction_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($invoice_report_view->transaction_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($invoice_report_view->customer_name->Visible) { // customer_name ?>
	<?php if ($invoice_report_view->SortUrl($invoice_report_view->customer_name) == "") { ?>
		<th data-name="customer_name" class="<?php echo $invoice_report_view->customer_name->HeaderCellClass() ?>"><div id="elh_invoice_report_view_customer_name" class="invoice_report_view_customer_name"><div class="ewTableHeaderCaption"><?php echo $invoice_report_view->customer_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="customer_name" class="<?php echo $invoice_report_view->customer_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $invoice_report_view->SortUrl($invoice_report_view->customer_name) ?>',1);"><div id="elh_invoice_report_view_customer_name" class="invoice_report_view_customer_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $invoice_report_view->customer_name->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($invoice_report_view->customer_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($invoice_report_view->customer_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($invoice_report_view->reference_customer->Visible) { // reference_customer ?>
	<?php if ($invoice_report_view->SortUrl($invoice_report_view->reference_customer) == "") { ?>
		<th data-name="reference_customer" class="<?php echo $invoice_report_view->reference_customer->HeaderCellClass() ?>"><div id="elh_invoice_report_view_reference_customer" class="invoice_report_view_reference_customer"><div class="ewTableHeaderCaption"><?php echo $invoice_report_view->reference_customer->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="reference_customer" class="<?php echo $invoice_report_view->reference_customer->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $invoice_report_view->SortUrl($invoice_report_view->reference_customer) ?>',1);"><div id="elh_invoice_report_view_reference_customer" class="invoice_report_view_reference_customer">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $invoice_report_view->reference_customer->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($invoice_report_view->reference_customer->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($invoice_report_view->reference_customer->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($invoice_report_view->created_employee->Visible) { // created_employee ?>
	<?php if ($invoice_report_view->SortUrl($invoice_report_view->created_employee) == "") { ?>
		<th data-name="created_employee" class="<?php echo $invoice_report_view->created_employee->HeaderCellClass() ?>"><div id="elh_invoice_report_view_created_employee" class="invoice_report_view_created_employee"><div class="ewTableHeaderCaption"><?php echo $invoice_report_view->created_employee->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="created_employee" class="<?php echo $invoice_report_view->created_employee->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $invoice_report_view->SortUrl($invoice_report_view->created_employee) ?>',1);"><div id="elh_invoice_report_view_created_employee" class="invoice_report_view_created_employee">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $invoice_report_view->created_employee->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($invoice_report_view->created_employee->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($invoice_report_view->created_employee->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($invoice_report_view->payment_status->Visible) { // payment_status ?>
	<?php if ($invoice_report_view->SortUrl($invoice_report_view->payment_status) == "") { ?>
		<th data-name="payment_status" class="<?php echo $invoice_report_view->payment_status->HeaderCellClass() ?>"><div id="elh_invoice_report_view_payment_status" class="invoice_report_view_payment_status"><div class="ewTableHeaderCaption"><?php echo $invoice_report_view->payment_status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="payment_status" class="<?php echo $invoice_report_view->payment_status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $invoice_report_view->SortUrl($invoice_report_view->payment_status) ?>',1);"><div id="elh_invoice_report_view_payment_status" class="invoice_report_view_payment_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $invoice_report_view->payment_status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($invoice_report_view->payment_status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($invoice_report_view->payment_status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($invoice_report_view->invoice_amount->Visible) { // invoice_amount ?>
	<?php if ($invoice_report_view->SortUrl($invoice_report_view->invoice_amount) == "") { ?>
		<th data-name="invoice_amount" class="<?php echo $invoice_report_view->invoice_amount->HeaderCellClass() ?>"><div id="elh_invoice_report_view_invoice_amount" class="invoice_report_view_invoice_amount"><div class="ewTableHeaderCaption"><?php echo $invoice_report_view->invoice_amount->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="invoice_amount" class="<?php echo $invoice_report_view->invoice_amount->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $invoice_report_view->SortUrl($invoice_report_view->invoice_amount) ?>',1);"><div id="elh_invoice_report_view_invoice_amount" class="invoice_report_view_invoice_amount">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $invoice_report_view->invoice_amount->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($invoice_report_view->invoice_amount->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($invoice_report_view->invoice_amount->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$invoice_report_view_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($invoice_report_view->ExportAll && $invoice_report_view->Export <> "") {
	$invoice_report_view_list->StopRec = $invoice_report_view_list->TotalRecs;
} else {

	// Set the last record to display
	if ($invoice_report_view_list->TotalRecs > $invoice_report_view_list->StartRec + $invoice_report_view_list->DisplayRecs - 1)
		$invoice_report_view_list->StopRec = $invoice_report_view_list->StartRec + $invoice_report_view_list->DisplayRecs - 1;
	else
		$invoice_report_view_list->StopRec = $invoice_report_view_list->TotalRecs;
}
$invoice_report_view_list->RecCnt = $invoice_report_view_list->StartRec - 1;
if ($invoice_report_view_list->Recordset && !$invoice_report_view_list->Recordset->EOF) {
	$invoice_report_view_list->Recordset->MoveFirst();
	$bSelectLimit = $invoice_report_view_list->UseSelectLimit;
	if (!$bSelectLimit && $invoice_report_view_list->StartRec > 1)
		$invoice_report_view_list->Recordset->Move($invoice_report_view_list->StartRec - 1);
} elseif (!$invoice_report_view->AllowAddDeleteRow && $invoice_report_view_list->StopRec == 0) {
	$invoice_report_view_list->StopRec = $invoice_report_view->GridAddRowCount;
}

// Initialize aggregate
$invoice_report_view->RowType = EW_ROWTYPE_AGGREGATEINIT;
$invoice_report_view->ResetAttrs();
$invoice_report_view_list->RenderRow();
while ($invoice_report_view_list->RecCnt < $invoice_report_view_list->StopRec) {
	$invoice_report_view_list->RecCnt++;
	if (intval($invoice_report_view_list->RecCnt) >= intval($invoice_report_view_list->StartRec)) {
		$invoice_report_view_list->RowCnt++;

		// Set up key count
		$invoice_report_view_list->KeyCount = $invoice_report_view_list->RowIndex;

		// Init row class and style
		$invoice_report_view->ResetAttrs();
		$invoice_report_view->CssClass = "";
		if ($invoice_report_view->CurrentAction == "gridadd") {
		} else {
			$invoice_report_view_list->LoadRowValues($invoice_report_view_list->Recordset); // Load row values
		}
		$invoice_report_view->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$invoice_report_view->RowAttrs = array_merge($invoice_report_view->RowAttrs, array('data-rowindex'=>$invoice_report_view_list->RowCnt, 'id'=>'r' . $invoice_report_view_list->RowCnt . '_invoice_report_view', 'data-rowtype'=>$invoice_report_view->RowType));
		...
		$invoice_report_view_list->RenderListOptions();
?>
	<tr<?php echo $invoice_report_view->RowAttributes() ?>>
<?php

// Render list options (body, left)
$invoice_report_view_list->ListOptions->Render("body", "left", $invoice_report_view_list->RowCnt);
?>
	<?php if ($invoice_report_view->invoice_no->Visible) { // invoice_no ?>
		<td data-name="invoice_no"<?php echo $invoice_report_view->invoice_no->CellAttributes() ?>>
<span id="el<?php echo $invoice_report_view_list->RowCnt ?>_invoice_report_view_invoice_no" class="invoice_report_view_invoice_no">
<span<?php echo $invoice_report_view->invoice_no->ViewAttributes() ?>>
<?php echo $invoice_report_view->invoice_no->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($invoice_report_view->transaction_date->Visible) { // transaction_date ?>
		<td data-name="transaction_date"<?php echo $invoice_report_view->transaction_date->CellAttributes() ?>>
<span id="el<?php echo $invoice_report_view_list->RowCnt ?>_invoice_report_view_transaction_date" class="invoice_report_view_transaction_date">
<span<?php echo $invoice_report_view->transaction_date->ViewAttributes() ?>>
<?php echo $invoice_report_view->transaction_date->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($invoice_report_view->customer_name->Visible) { // customer_name ?>
		<td data-name="customer_name"<?php echo $invoice_report_view->customer_name->CellAttributes() ?>>
<span id="el<?php echo $invoice_report_view_list->RowCnt ?>_invoice_report_view_customer_name" class="invoice_report_view_customer_name">
<span<?php echo $invoice_report_view->customer_name->ViewAttributes() ?>>
<?php echo $invoice_report_view->customer_name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($invoice_report_view->reference_customer->Visible) { // reference_customer ?>
		<td data-name="reference_customer"<?php echo $invoice_report_view->reference_customer->CellAttributes() ?>>
<span id="el<?php echo $invoice_report_view_list->RowCnt ?>_invoice_report_view_reference_customer" class="invoice_report_view_reference_customer">
<span<?php echo $invoice_report_view->reference_customer->ViewAttributes() ?>>
<?php echo $invoice_report_view->reference_customer->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($invoice_report_view->created_employee->Visible) { // created_employee ?>
		<td data-name="created_employee"<?php echo $invoice_report_view->created_employee->CellAttributes() ?>>
<span id="el<?php echo $invoice_report_view_list->RowCnt ?>_invoice_report_view_created_employee" class="invoice_report_view_created_employee">
<span<?php echo $invoice_report_view->created_employee->ViewAttributes() ?>>
<?php echo $invoice_report_view->created_employee->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($invoice_report_view->payment_status->Visible) { // payment_status ?>
		<td data-name="payment_status"<?php echo $invoice_report_view->payment_status->CellAttributes() ?>>
<span id="el<?php echo $invoice_report_view_list->RowCnt ?>_invoice_report_view_payment_status" class="invoice_report_view_payment_status">
<span<?php echo $invoice_report_view->payment_status->ViewAttributes() ?>>
<?php echo $invoice_report_view->payment_status->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($invoice_report_view->invoice_amount->Visible) { // invoice_amount ?>
		<td data-name="invoice_amount"<?php echo $invoice_report_view->invoice_amount->CellAttributes() ?>>
<span id="el<?php echo $invoice_report_view_list->RowCnt ?>_invoice_report_view_invoice_amount" class="invoice_report_view_invoice_amount">
<span<?php echo $invoice_report_view->invoice_amount->ViewAttributes() ?>>
<?php echo $invoice_report_view->invoice_amount->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$invoice_report_view_list->ListOptions->Render("body", "right", $invoice_report_view_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($invoice_report_view->CurrentAction <> "gridadd")
		$invoice_report_view_list->Recordset->MoveNext();
}
?>
</tbody>
<?php
...
$invoice_report_view->RowType = EW_ROWTYPE_AGGREGATE;
$invoice_report_view->ResetAttrs();
$invoice_report_view_list->RenderRow();
?>
<?php if ($invoice_report_view_list->TotalRecs > 0 && ($invoice_report_view->CurrentAction <> "gridadd" && $invoice_report_view->CurrentAction <> "gridedit")) { ?>
<tfoot><!-- Table footer -->
...
$invoice_report_view_list->ListOptions->Render("footer", "left");
?>
	<?php if ($invoice_report_view->invoice_no->Visible) { // invoice_no ?>
		<td data-name="invoice_no" class="<?php echo $invoice_report_view->invoice_no->FooterCellClass() ?>"><span id="elf_invoice_report_view_invoice_no" class="invoice_report_view_invoice_no">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($invoice_report_view->transaction_date->Visible) { // transaction_date ?>
		<td data-name="transaction_date" class="<?php echo $invoice_report_view->transaction_date->FooterCellClass() ?>"><span id="elf_invoice_report_view_transaction_date" class="invoice_report_view_transaction_date">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($invoice_report_view->customer_name->Visible) { // customer_name ?>
		<td data-name="customer_name" class="<?php echo $invoice_report_view->customer_name->FooterCellClass() ?>"><span id="elf_invoice_report_view_customer_name" class="invoice_report_view_customer_name">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($invoice_report_view->reference_customer->Visible) { // reference_customer ?>
		<td data-name="reference_customer" class="<?php echo $invoice_report_view->reference_customer->FooterCellClass() ?>"><span id="elf_invoice_report_view_reference_customer" class="invoice_report_view_reference_customer">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($invoice_report_view->created_employee->Visible) { // created_employee ?>
		<td data-name="created_employee" class="<?php echo $invoice_report_view->created_employee->FooterCellClass() ?>"><span id="elf_invoice_report_view_created_employee" class="invoice_report_view_created_employee">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($invoice_report_view->payment_status->Visible) { // payment_status ?>
		<td data-name="payment_status" class="<?php echo $invoice_report_view->payment_status->FooterCellClass() ?>"><span id="elf_invoice_report_view_payment_status" class="invoice_report_view_payment_status">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($invoice_report_view->invoice_amount->Visible) { // invoice_amount ?>
		<td data-name="invoice_amount" class="<?php echo $invoice_report_view->invoice_amount->FooterCellClass() ?>"><span id="elf_invoice_report_view_invoice_amount" class="invoice_report_view_invoice_amount">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span><span class="ewAggregateValue">
<?php echo $invoice_report_view->invoice_amount->ViewValue ?></span>
		</span></td>
	<?php } ?>
<?php

// Render list options (footer, right)
$invoice_report_view_list->ListOptions->Render("footer", "right");
?>
	</tr>
</tfoot>
<?php } ?>
...
<?php } ?>
<?php if ($invoice_report_view->CurrentAction == "") { ?>
...
// Close recordset
if ($invoice_report_view_list->Recordset)
	$invoice_report_view_list->Recordset->Close();
?>
<?php if ($invoice_report_view->Export == "") { ?>
<div class="box-footer ewGridLowerPanel">
<?php if ($invoice_report_view->CurrentAction <> "gridadd" && $invoice_report_view->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($invoice_report_view_list->Pager)) $invoice_report_view_list->Pager = new cPrevNextPager($invoice_report_view_list->StartRec, $invoice_report_view_list->DisplayRecs, $invoice_report_view_list->TotalRecs, $invoice_report_view_list->AutoHidePager) ?>
<?php if ($invoice_report_view_list->Pager->RecordCount > 0 && $invoice_report_view_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($invoice_report_view_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $invoice_report_view_list->PageUrl() ?>start=<?php echo $invoice_report_view_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($invoice_report_view_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $invoice_report_view_list->PageUrl() ?>start=<?php echo $invoice_report_view_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $invoice_report_view_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($invoice_report_view_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $invoice_report_view_list->PageUrl() ?>start=<?php echo $invoice_report_view_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($invoice_report_view_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $invoice_report_view_list->PageUrl() ?>start=<?php echo $invoice_report_view_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $invoice_report_view_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($invoice_report_view_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $invoice_report_view_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $invoice_report_view_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $invoice_report_view_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($invoice_report_view_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($invoice_report_view_list->TotalRecs == 0 && $invoice_report_view->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($invoice_report_view_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($invoice_report_view->Export == "") { ?>
<script type="text/javascript">
finvoice_report_viewlistsrch.FilterList = <?php echo $invoice_report_view_list->GetFilterList() ?>;
finvoice_report_viewlistsrch.Init();
finvoice_report_viewlist.Init();
</script>
<?php } ?>
<?php
$invoice_report_view_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($invoice_report_view->Export == "") { ?>
...
<?php
$invoice_report_view_list->Page_Terminate();
?>
