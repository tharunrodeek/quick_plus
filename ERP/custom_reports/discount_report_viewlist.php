...
<?php include_once "discount_report_viewinfo.php" ?>
...
$discount_report_view_list = NULL; // Initialize page object first

class cdiscount_report_view_list extends cdiscount_report_view {
	...
	// Table name
	var $TableName = 'discount_report_view';

	// Page object name
	var $PageObjName = 'discount_report_view_list';

	// Grid form hidden field names
	var $FormName = 'fdiscount_report_viewlist';
	...
	function __construct() {
		...
		// Table object (discount_report_view)
		if (!isset($GLOBALS["discount_report_view"]) || get_class($GLOBALS["discount_report_view"]) == "cdiscount_report_view") {
			$GLOBALS["discount_report_view"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["discount_report_view"];
		}
		...
		$this->AddUrl = "discount_report_viewadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "discount_report_viewdelete.php";
		$this->MultiUpdateUrl = "discount_report_viewupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'discount_report_view', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fdiscount_report_viewlistsrch";
	...
	function Page_Init() {
		...
		$this->total_discount->SetVisibility();
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $discount_report_view;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($discount_report_view);
				$doc->Text = $sContent;
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fdiscount_report_viewlistsrch", $filters);
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->total_discount, $Default, FALSE); // total_discount
		...
		if (!$Default && $this->Command == "search") {
			...
			$this->total_discount->AdvancedSearch->Save(); // total_discount
	...
	function CheckSearchParms() {
		...
		if ($this->quantity->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->total_discount->AdvancedSearch->IssetSession())
			return TRUE;
	...
	function ResetAdvancedSearchParms() {
		...
		$this->total_discount->AdvancedSearch->UnsetSession();
	...
	function RestoreSearchParms() {
		...
		$this->total_discount->AdvancedSearch->Load();
	...
	function SetupSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->total_discount); // total_discount
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fdiscount_report_viewlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fdiscount_report_viewlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fdiscount_report_viewlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fdiscount_report_viewlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function LoadSearchValues() {
		...
		// total_discount
		$this->total_discount->AdvancedSearch->SearchValue = @$_GET["x_total_discount"];
		if ($this->total_discount->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->total_discount->AdvancedSearch->SearchOperator = @$_GET["z_total_discount"];
	...
	function RenderRow() {
		...
		$this->stock_id->ViewCustomAttributes = "";
			...
			$this->total_discount->EditValue = ew_HtmlEncode($this->total_discount->AdvancedSearch->SearchValue);
			...
			$this->total_discount->Total = 0; // Initialize total
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_discount_report_view\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_discount_report_view',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fdiscount_report_viewlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
	...
	function ExportQueryString() {
		...
		$this->AddSearchQueryString($sQry, $this->total_discount); // total_discount
...
// Create page object
if (!isset($discount_report_view_list)) $discount_report_view_list = new cdiscount_report_view_list();

// Page init
$discount_report_view_list->Page_Init();

// Page main
$discount_report_view_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$discount_report_view_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($discount_report_view->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = fdiscount_report_viewlist = new ew_Form("fdiscount_report_viewlist", "list");
fdiscount_report_viewlist.FormKeyCountName = '<?php echo $discount_report_view_list->FormKeyCountName ?>';

// Form_CustomValidate event
fdiscount_report_viewlist.Form_CustomValidate = 
...
fdiscount_report_viewlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fdiscount_report_viewlist.Lists["x_customer_name[]"] = {"LinkField":"x_name","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_debtors_master"};
fdiscount_report_viewlist.Lists["x_customer_name[]"].Data = "<?php echo $discount_report_view_list->customer_name->LookupFilterQuery(FALSE, "list") ?>";
fdiscount_report_viewlist.Lists["x_created_employee[]"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_id","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_users"};
fdiscount_report_viewlist.Lists["x_created_employee[]"].Data = "<?php echo $discount_report_view_list->created_employee->LookupFilterQuery(FALSE, "list") ?>";
fdiscount_report_viewlist.Lists["x_stock_id"] = {"LinkField":"x_stock_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","x_long_description","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_stock_master"};
fdiscount_report_viewlist.Lists["x_stock_id"].Data = "<?php echo $discount_report_view_list->stock_id->LookupFilterQuery(FALSE, "list") ?>";

// Form object for search
var CurrentSearchForm = fdiscount_report_viewlistsrch = new ew_Form("fdiscount_report_viewlistsrch");

// Validate function for search
fdiscount_report_viewlistsrch.Validate = function(fobj) {
	...
	if (elm && !ew_CheckDateDef(elm.value))
		return this.OnError(elm, "<?php echo ew_JsEncode2($discount_report_view->transaction_date->FldErrMsg()) ?>");
...
// Form_CustomValidate event
fdiscount_report_viewlistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
fdiscount_report_viewlistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fdiscount_report_viewlistsrch.Lists["x_customer_name[]"] = {"LinkField":"x_name","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_debtors_master"};
fdiscount_report_viewlistsrch.Lists["x_customer_name[]"].Data = "<?php echo $discount_report_view_list->customer_name->LookupFilterQuery(FALSE, "extbs") ?>";
fdiscount_report_viewlistsrch.Lists["x_created_employee[]"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_id","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_users"};
fdiscount_report_viewlistsrch.Lists["x_created_employee[]"].Data = "<?php echo $discount_report_view_list->created_employee->LookupFilterQuery(FALSE, "extbs") ?>";
fdiscount_report_viewlistsrch.Lists["x_stock_id"] = {"LinkField":"x_stock_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","x_long_description","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_stock_master"};
fdiscount_report_viewlistsrch.Lists["x_stock_id"].Data = "<?php echo $discount_report_view_list->stock_id->LookupFilterQuery(FALSE, "extbs") ?>";
</script>
<script type="text/javascript">
...
<?php if ($discount_report_view->Export == "") { ?>
<div class="ewToolbar">
<?php if ($discount_report_view_list->TotalRecs > 0 && $discount_report_view_list->ExportOptions->Visible()) { ?>
<?php $discount_report_view_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($discount_report_view_list->SearchOptions->Visible()) { ?>
<?php $discount_report_view_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($discount_report_view_list->FilterOptions->Visible()) { ?>
<?php $discount_report_view_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $discount_report_view_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($discount_report_view_list->TotalRecs <= 0)
			$discount_report_view_list->TotalRecs = $discount_report_view->ListRecordCount();
	} else {
		if (!$discount_report_view_list->Recordset && ($discount_report_view_list->Recordset = $discount_report_view_list->LoadRecordset()))
			$discount_report_view_list->TotalRecs = $discount_report_view_list->Recordset->RecordCount();
	}
	$discount_report_view_list->StartRec = 1;
	if ($discount_report_view_list->DisplayRecs <= 0 || ($discount_report_view->Export <> "" && $discount_report_view->ExportAll)) // Display all records
		$discount_report_view_list->DisplayRecs = $discount_report_view_list->TotalRecs;
	if (!($discount_report_view->Export <> "" && $discount_report_view->ExportAll))
		$discount_report_view_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$discount_report_view_list->Recordset = $discount_report_view_list->LoadRecordset($discount_report_view_list->StartRec-1, $discount_report_view_list->DisplayRecs);

	// Set no record found message
	if ($discount_report_view->CurrentAction == "" && $discount_report_view_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$discount_report_view_list->setWarningMessage(ew_DeniedMsg());
		if ($discount_report_view_list->SearchWhere == "0=101")
			$discount_report_view_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$discount_report_view_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$discount_report_view_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($discount_report_view->Export == "" && $discount_report_view->CurrentAction == "") { ?>
<form name="fdiscount_report_viewlistsrch" id="fdiscount_report_viewlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($discount_report_view_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fdiscount_report_viewlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="discount_report_view">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$discount_report_view_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$discount_report_view->RowType = EW_ROWTYPE_SEARCH;

// Render row
$discount_report_view->ResetAttrs();
$discount_report_view_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($discount_report_view->invoice_no->Visible) { // invoice_no ?>
	<div id="xsc_invoice_no" class="ewCell form-group">
		<label for="x_invoice_no" class="ewSearchCaption ewLabel"><?php echo $discount_report_view->invoice_no->FldCaption() ?></label>
...
<input type="text" data-table="discount_report_view" data-field="x_invoice_no" name="x_invoice_no" id="x_invoice_no" size="30" maxlength="60" placeholder="<?php echo ew_HtmlEncode($discount_report_view->invoice_no->getPlaceHolder()) ?>" value="<?php echo $discount_report_view->invoice_no->EditValue ?>"<?php echo $discount_report_view->invoice_no->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_2" class="ewRow">
<?php if ($discount_report_view->transaction_date->Visible) { // transaction_date ?>
	<div id="xsc_transaction_date" class="ewCell form-group">
		<label for="x_transaction_date" class="ewSearchCaption ewLabel"><?php echo $discount_report_view->transaction_date->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<input type="text" data-table="discount_report_view" data-field="x_transaction_date" data-format="2" name="x_transaction_date" id="x_transaction_date" placeholder="<?php echo ew_HtmlEncode($discount_report_view->transaction_date->getPlaceHolder()) ?>" value="<?php echo $discount_report_view->transaction_date->EditValue ?>"<?php echo $discount_report_view->transaction_date->EditAttributes() ?>>
<?php if (!$discount_report_view->transaction_date->ReadOnly && !$discount_report_view->transaction_date->Disabled && !isset($discount_report_view->transaction_date->EditAttrs["readonly"]) && !isset($discount_report_view->transaction_date->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("fdiscount_report_viewlistsrch", "x_transaction_date", {"ignoreReadonly":true,"useCurrent":false,"format":2});
</script>
<?php } ?>
</span>
...
<input type="text" data-table="discount_report_view" data-field="x_transaction_date" data-format="2" name="y_transaction_date" id="y_transaction_date" placeholder="<?php echo ew_HtmlEncode($discount_report_view->transaction_date->getPlaceHolder()) ?>" value="<?php echo $discount_report_view->transaction_date->EditValue2 ?>"<?php echo $discount_report_view->transaction_date->EditAttributes() ?>>
<?php if (!$discount_report_view->transaction_date->ReadOnly && !$discount_report_view->transaction_date->Disabled && !isset($discount_report_view->transaction_date->EditAttrs["readonly"]) && !isset($discount_report_view->transaction_date->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("fdiscount_report_viewlistsrch", "y_transaction_date", {"ignoreReadonly":true,"useCurrent":false,"format":2});
</script>
<?php } ?>
</span>
	</div>
...
<?php if ($discount_report_view->customer_name->Visible) { // customer_name ?>
	<div id="xsc_customer_name" class="ewCell form-group">
		<label for="x_customer_name" class="ewSearchCaption ewLabel"><?php echo $discount_report_view->customer_name->FldCaption() ?></label>
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_customer_name"><?php echo (strval($discount_report_view->customer_name->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $discount_report_view->customer_name->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($discount_report_view->customer_name->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_customer_name[]',m:1,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($discount_report_view->customer_name->ReadOnly || $discount_report_view->customer_name->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="discount_report_view" data-field="x_customer_name" data-multiple="1" data-lookup="1" data-value-separator="<?php echo $discount_report_view->customer_name->DisplayValueSeparatorAttribute() ?>" name="x_customer_name[]" id="x_customer_name[]" value="<?php echo $discount_report_view->customer_name->AdvancedSearch->SearchValue ?>"<?php echo $discount_report_view->customer_name->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_4" class="ewRow">
<?php if ($discount_report_view->created_employee->Visible) { // created_employee ?>
	<div id="xsc_created_employee" class="ewCell form-group">
		<label for="x_created_employee" class="ewSearchCaption ewLabel"><?php echo $discount_report_view->created_employee->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_created_employee"><?php echo (strval($discount_report_view->created_employee->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $discount_report_view->created_employee->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($discount_report_view->created_employee->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_created_employee[]',m:1,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($discount_report_view->created_employee->ReadOnly || $discount_report_view->created_employee->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="discount_report_view" data-field="x_created_employee" data-multiple="1" data-lookup="1" data-value-separator="<?php echo $discount_report_view->created_employee->DisplayValueSeparatorAttribute() ?>" name="x_created_employee[]" id="x_created_employee[]" value="<?php echo $discount_report_view->created_employee->AdvancedSearch->SearchValue ?>"<?php echo $discount_report_view->created_employee->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_5" class="ewRow">
<?php if ($discount_report_view->stock_id->Visible) { // stock_id ?>
	<div id="xsc_stock_id" class="ewCell form-group">
		<label for="x_stock_id" class="ewSearchCaption ewLabel"><?php echo $discount_report_view->stock_id->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_stock_id"><?php echo (strval($discount_report_view->stock_id->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $discount_report_view->stock_id->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($discount_report_view->stock_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_stock_id',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($discount_report_view->stock_id->ReadOnly || $discount_report_view->stock_id->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="discount_report_view" data-field="x_stock_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $discount_report_view->stock_id->DisplayValueSeparatorAttribute() ?>" name="x_stock_id" id="x_stock_id" value="<?php echo $discount_report_view->stock_id->AdvancedSearch->SearchValue ?>"<?php echo $discount_report_view->stock_id->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_6" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($discount_report_view_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($discount_report_view_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $discount_report_view_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($discount_report_view_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($discount_report_view_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($discount_report_view_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($discount_report_view_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $discount_report_view_list->ShowPageHeader(); ?>
<?php
$discount_report_view_list->ShowMessage();
?>
<?php if ($discount_report_view_list->TotalRecs > 0 || $discount_report_view->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($discount_report_view_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> discount_report_view">
<form name="fdiscount_report_viewlist" id="fdiscount_report_viewlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($discount_report_view_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $discount_report_view_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="discount_report_view">
<div id="gmp_discount_report_view" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($discount_report_view_list->TotalRecs > 0 || $discount_report_view->CurrentAction == "gridedit") { ?>
<table id="tbl_discount_report_viewlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$discount_report_view_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$discount_report_view_list->RenderListOptions();

// Render list options (header, left)
$discount_report_view_list->ListOptions->Render("header", "left");
?>
<?php if ($discount_report_view->invoice_no->Visible) { // invoice_no ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->invoice_no) == "") { ?>
		<th data-name="invoice_no" class="<?php echo $discount_report_view->invoice_no->HeaderCellClass() ?>"><div id="elh_discount_report_view_invoice_no" class="discount_report_view_invoice_no"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->invoice_no->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="invoice_no" class="<?php echo $discount_report_view->invoice_no->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->invoice_no) ?>',1);"><div id="elh_discount_report_view_invoice_no" class="discount_report_view_invoice_no">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->invoice_no->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->invoice_no->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->invoice_no->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($discount_report_view->transaction_date->Visible) { // transaction_date ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->transaction_date) == "") { ?>
		<th data-name="transaction_date" class="<?php echo $discount_report_view->transaction_date->HeaderCellClass() ?>"><div id="elh_discount_report_view_transaction_date" class="discount_report_view_transaction_date"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->transaction_date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="transaction_date" class="<?php echo $discount_report_view->transaction_date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->transaction_date) ?>',1);"><div id="elh_discount_report_view_transaction_date" class="discount_report_view_transaction_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->transaction_date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->transaction_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->transaction_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($discount_report_view->description->Visible) { // description ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->description) == "") { ?>
		<th data-name="description" class="<?php echo $discount_report_view->description->HeaderCellClass() ?>"><div id="elh_discount_report_view_description" class="discount_report_view_description"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->description->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="description" class="<?php echo $discount_report_view->description->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->description) ?>',1);"><div id="elh_discount_report_view_description" class="discount_report_view_description">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->description->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->description->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->description->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($discount_report_view->customer_name->Visible) { // customer_name ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->customer_name) == "") { ?>
		<th data-name="customer_name" class="<?php echo $discount_report_view->customer_name->HeaderCellClass() ?>"><div id="elh_discount_report_view_customer_name" class="discount_report_view_customer_name"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->customer_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="customer_name" class="<?php echo $discount_report_view->customer_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->customer_name) ?>',1);"><div id="elh_discount_report_view_customer_name" class="discount_report_view_customer_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->customer_name->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->customer_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->customer_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($discount_report_view->reference_customer->Visible) { // reference_customer ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->reference_customer) == "") { ?>
		<th data-name="reference_customer" class="<?php echo $discount_report_view->reference_customer->HeaderCellClass() ?>"><div id="elh_discount_report_view_reference_customer" class="discount_report_view_reference_customer"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->reference_customer->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="reference_customer" class="<?php echo $discount_report_view->reference_customer->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->reference_customer) ?>',1);"><div id="elh_discount_report_view_reference_customer" class="discount_report_view_reference_customer">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->reference_customer->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->reference_customer->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->reference_customer->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($discount_report_view->created_employee->Visible) { // created_employee ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->created_employee) == "") { ?>
		<th data-name="created_employee" class="<?php echo $discount_report_view->created_employee->HeaderCellClass() ?>"><div id="elh_discount_report_view_created_employee" class="discount_report_view_created_employee"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->created_employee->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="created_employee" class="<?php echo $discount_report_view->created_employee->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->created_employee) ?>',1);"><div id="elh_discount_report_view_created_employee" class="discount_report_view_created_employee">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->created_employee->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->created_employee->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->created_employee->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($discount_report_view->discount_amount->Visible) { // discount_amount ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->discount_amount) == "") { ?>
		<th data-name="discount_amount" class="<?php echo $discount_report_view->discount_amount->HeaderCellClass() ?>"><div id="elh_discount_report_view_discount_amount" class="discount_report_view_discount_amount"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->discount_amount->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="discount_amount" class="<?php echo $discount_report_view->discount_amount->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->discount_amount) ?>',1);"><div id="elh_discount_report_view_discount_amount" class="discount_report_view_discount_amount">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->discount_amount->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->discount_amount->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->discount_amount->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($discount_report_view->quantity->Visible) { // quantity ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->quantity) == "") { ?>
		<th data-name="quantity" class="<?php echo $discount_report_view->quantity->HeaderCellClass() ?>"><div id="elh_discount_report_view_quantity" class="discount_report_view_quantity"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->quantity->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="quantity" class="<?php echo $discount_report_view->quantity->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->quantity) ?>',1);"><div id="elh_discount_report_view_quantity" class="discount_report_view_quantity">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->quantity->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->quantity->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->quantity->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($discount_report_view->total_discount->Visible) { // total_discount ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->total_discount) == "") { ?>
		<th data-name="total_discount" class="<?php echo $discount_report_view->total_discount->HeaderCellClass() ?>"><div id="elh_discount_report_view_total_discount" class="discount_report_view_total_discount"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->total_discount->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="total_discount" class="<?php echo $discount_report_view->total_discount->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->total_discount) ?>',1);"><div id="elh_discount_report_view_total_discount" class="discount_report_view_total_discount">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->total_discount->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->total_discount->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->total_discount->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($discount_report_view->stock_id->Visible) { // stock_id ?>
	<?php if ($discount_report_view->SortUrl($discount_report_view->stock_id) == "") { ?>
		<th data-name="stock_id" class="<?php echo $discount_report_view->stock_id->HeaderCellClass() ?>"><div id="elh_discount_report_view_stock_id" class="discount_report_view_stock_id"><div class="ewTableHeaderCaption"><?php echo $discount_report_view->stock_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="stock_id" class="<?php echo $discount_report_view->stock_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $discount_report_view->SortUrl($discount_report_view->stock_id) ?>',1);"><div id="elh_discount_report_view_stock_id" class="discount_report_view_stock_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $discount_report_view->stock_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($discount_report_view->stock_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($discount_report_view->stock_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$discount_report_view_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($discount_report_view->ExportAll && $discount_report_view->Export <> "") {
	$discount_report_view_list->StopRec = $discount_report_view_list->TotalRecs;
} else {

	// Set the last record to display
	if ($discount_report_view_list->TotalRecs > $discount_report_view_list->StartRec + $discount_report_view_list->DisplayRecs - 1)
		$discount_report_view_list->StopRec = $discount_report_view_list->StartRec + $discount_report_view_list->DisplayRecs - 1;
	else
		$discount_report_view_list->StopRec = $discount_report_view_list->TotalRecs;
}
$discount_report_view_list->RecCnt = $discount_report_view_list->StartRec - 1;
if ($discount_report_view_list->Recordset && !$discount_report_view_list->Recordset->EOF) {
	$discount_report_view_list->Recordset->MoveFirst();
	$bSelectLimit = $discount_report_view_list->UseSelectLimit;
	if (!$bSelectLimit && $discount_report_view_list->StartRec > 1)
		$discount_report_view_list->Recordset->Move($discount_report_view_list->StartRec - 1);
} elseif (!$discount_report_view->AllowAddDeleteRow && $discount_report_view_list->StopRec == 0) {
	$discount_report_view_list->StopRec = $discount_report_view->GridAddRowCount;
}

// Initialize aggregate
$discount_report_view->RowType = EW_ROWTYPE_AGGREGATEINIT;
$discount_report_view->ResetAttrs();
$discount_report_view_list->RenderRow();
while ($discount_report_view_list->RecCnt < $discount_report_view_list->StopRec) {
	$discount_report_view_list->RecCnt++;
	if (intval($discount_report_view_list->RecCnt) >= intval($discount_report_view_list->StartRec)) {
		$discount_report_view_list->RowCnt++;

		// Set up key count
		$discount_report_view_list->KeyCount = $discount_report_view_list->RowIndex;

		// Init row class and style
		$discount_report_view->ResetAttrs();
		$discount_report_view->CssClass = "";
		if ($discount_report_view->CurrentAction == "gridadd") {
		} else {
			$discount_report_view_list->LoadRowValues($discount_report_view_list->Recordset); // Load row values
		}
		$discount_report_view->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$discount_report_view->RowAttrs = array_merge($discount_report_view->RowAttrs, array('data-rowindex'=>$discount_report_view_list->RowCnt, 'id'=>'r' . $discount_report_view_list->RowCnt . '_discount_report_view', 'data-rowtype'=>$discount_report_view->RowType));
		...
		$discount_report_view_list->RenderListOptions();
?>
	<tr<?php echo $discount_report_view->RowAttributes() ?>>
<?php

// Render list options (body, left)
$discount_report_view_list->ListOptions->Render("body", "left", $discount_report_view_list->RowCnt);
?>
	<?php if ($discount_report_view->invoice_no->Visible) { // invoice_no ?>
		<td data-name="invoice_no"<?php echo $discount_report_view->invoice_no->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_invoice_no" class="discount_report_view_invoice_no">
<span<?php echo $discount_report_view->invoice_no->ViewAttributes() ?>>
<?php echo $discount_report_view->invoice_no->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($discount_report_view->transaction_date->Visible) { // transaction_date ?>
		<td data-name="transaction_date"<?php echo $discount_report_view->transaction_date->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_transaction_date" class="discount_report_view_transaction_date">
<span<?php echo $discount_report_view->transaction_date->ViewAttributes() ?>>
<?php echo $discount_report_view->transaction_date->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($discount_report_view->description->Visible) { // description ?>
		<td data-name="description"<?php echo $discount_report_view->description->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_description" class="discount_report_view_description">
<span<?php echo $discount_report_view->description->ViewAttributes() ?>>
<?php echo $discount_report_view->description->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($discount_report_view->customer_name->Visible) { // customer_name ?>
		<td data-name="customer_name"<?php echo $discount_report_view->customer_name->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_customer_name" class="discount_report_view_customer_name">
<span<?php echo $discount_report_view->customer_name->ViewAttributes() ?>>
<?php echo $discount_report_view->customer_name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($discount_report_view->reference_customer->Visible) { // reference_customer ?>
		<td data-name="reference_customer"<?php echo $discount_report_view->reference_customer->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_reference_customer" class="discount_report_view_reference_customer">
<span<?php echo $discount_report_view->reference_customer->ViewAttributes() ?>>
<?php echo $discount_report_view->reference_customer->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($discount_report_view->created_employee->Visible) { // created_employee ?>
		<td data-name="created_employee"<?php echo $discount_report_view->created_employee->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_created_employee" class="discount_report_view_created_employee">
<span<?php echo $discount_report_view->created_employee->ViewAttributes() ?>>
<?php echo $discount_report_view->created_employee->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($discount_report_view->discount_amount->Visible) { // discount_amount ?>
		<td data-name="discount_amount"<?php echo $discount_report_view->discount_amount->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_discount_amount" class="discount_report_view_discount_amount">
<span<?php echo $discount_report_view->discount_amount->ViewAttributes() ?>>
<?php echo $discount_report_view->discount_amount->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($discount_report_view->quantity->Visible) { // quantity ?>
		<td data-name="quantity"<?php echo $discount_report_view->quantity->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_quantity" class="discount_report_view_quantity">
<span<?php echo $discount_report_view->quantity->ViewAttributes() ?>>
<?php echo $discount_report_view->quantity->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($discount_report_view->total_discount->Visible) { // total_discount ?>
		<td data-name="total_discount"<?php echo $discount_report_view->total_discount->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_total_discount" class="discount_report_view_total_discount">
<span<?php echo $discount_report_view->total_discount->ViewAttributes() ?>>
<?php echo $discount_report_view->total_discount->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($discount_report_view->stock_id->Visible) { // stock_id ?>
		<td data-name="stock_id"<?php echo $discount_report_view->stock_id->CellAttributes() ?>>
<span id="el<?php echo $discount_report_view_list->RowCnt ?>_discount_report_view_stock_id" class="discount_report_view_stock_id">
<span<?php echo $discount_report_view->stock_id->ViewAttributes() ?>>
<?php echo $discount_report_view->stock_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$discount_report_view_list->ListOptions->Render("body", "right", $discount_report_view_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($discount_report_view->CurrentAction <> "gridadd")
		$discount_report_view_list->Recordset->MoveNext();
}
?>
</tbody>
<?php
...
$discount_report_view->RowType = EW_ROWTYPE_AGGREGATE;
$discount_report_view->ResetAttrs();
$discount_report_view_list->RenderRow();
?>
<?php if ($discount_report_view_list->TotalRecs > 0 && ($discount_report_view->CurrentAction <> "gridadd" && $discount_report_view->CurrentAction <> "gridedit")) { ?>
<tfoot><!-- Table footer -->
...
$discount_report_view_list->ListOptions->Render("footer", "left");
?>
	<?php if ($discount_report_view->invoice_no->Visible) { // invoice_no ?>
		<td data-name="invoice_no" class="<?php echo $discount_report_view->invoice_no->FooterCellClass() ?>"><span id="elf_discount_report_view_invoice_no" class="discount_report_view_invoice_no">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($discount_report_view->transaction_date->Visible) { // transaction_date ?>
		<td data-name="transaction_date" class="<?php echo $discount_report_view->transaction_date->FooterCellClass() ?>"><span id="elf_discount_report_view_transaction_date" class="discount_report_view_transaction_date">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($discount_report_view->description->Visible) { // description ?>
		<td data-name="description" class="<?php echo $discount_report_view->description->FooterCellClass() ?>"><span id="elf_discount_report_view_description" class="discount_report_view_description">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($discount_report_view->customer_name->Visible) { // customer_name ?>
		<td data-name="customer_name" class="<?php echo $discount_report_view->customer_name->FooterCellClass() ?>"><span id="elf_discount_report_view_customer_name" class="discount_report_view_customer_name">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($discount_report_view->reference_customer->Visible) { // reference_customer ?>
		<td data-name="reference_customer" class="<?php echo $discount_report_view->reference_customer->FooterCellClass() ?>"><span id="elf_discount_report_view_reference_customer" class="discount_report_view_reference_customer">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($discount_report_view->created_employee->Visible) { // created_employee ?>
		<td data-name="created_employee" class="<?php echo $discount_report_view->created_employee->FooterCellClass() ?>"><span id="elf_discount_report_view_created_employee" class="discount_report_view_created_employee">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($discount_report_view->discount_amount->Visible) { // discount_amount ?>
		<td data-name="discount_amount" class="<?php echo $discount_report_view->discount_amount->FooterCellClass() ?>"><span id="elf_discount_report_view_discount_amount" class="discount_report_view_discount_amount">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($discount_report_view->quantity->Visible) { // quantity ?>
		<td data-name="quantity" class="<?php echo $discount_report_view->quantity->FooterCellClass() ?>"><span id="elf_discount_report_view_quantity" class="discount_report_view_quantity">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($discount_report_view->total_discount->Visible) { // total_discount ?>
		<td data-name="total_discount" class="<?php echo $discount_report_view->total_discount->FooterCellClass() ?>"><span id="elf_discount_report_view_total_discount" class="discount_report_view_total_discount">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span><span class="ewAggregateValue">
<?php echo $discount_report_view->total_discount->ViewValue ?></span>
		</span></td>
	<?php } ?>
	<?php if ($discount_report_view->stock_id->Visible) { // stock_id ?>
		<td data-name="stock_id" class="<?php echo $discount_report_view->stock_id->FooterCellClass() ?>"><span id="elf_discount_report_view_stock_id" class="discount_report_view_stock_id">
		&nbsp;
		</span></td>
	<?php } ?>
<?php
...
$discount_report_view_list->ListOptions->Render("footer", "right");
?>
	</tr>
</tfoot>
<?php } ?>
...
<?php } ?>
<?php if ($discount_report_view->CurrentAction == "") { ?>
...
// Close recordset
if ($discount_report_view_list->Recordset)
	$discount_report_view_list->Recordset->Close();
?>
<?php if ($discount_report_view->Export == "") { ?>
<div class="box-footer ewGridLowerPanel">
<?php if ($discount_report_view->CurrentAction <> "gridadd" && $discount_report_view->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($discount_report_view_list->Pager)) $discount_report_view_list->Pager = new cPrevNextPager($discount_report_view_list->StartRec, $discount_report_view_list->DisplayRecs, $discount_report_view_list->TotalRecs, $discount_report_view_list->AutoHidePager) ?>
<?php if ($discount_report_view_list->Pager->RecordCount > 0 && $discount_report_view_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($discount_report_view_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $discount_report_view_list->PageUrl() ?>start=<?php echo $discount_report_view_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($discount_report_view_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $discount_report_view_list->PageUrl() ?>start=<?php echo $discount_report_view_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $discount_report_view_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($discount_report_view_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $discount_report_view_list->PageUrl() ?>start=<?php echo $discount_report_view_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($discount_report_view_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $discount_report_view_list->PageUrl() ?>start=<?php echo $discount_report_view_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $discount_report_view_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($discount_report_view_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $discount_report_view_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $discount_report_view_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $discount_report_view_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($discount_report_view_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($discount_report_view_list->TotalRecs == 0 && $discount_report_view->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($discount_report_view_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($discount_report_view->Export == "") { ?>
<script type="text/javascript">
fdiscount_report_viewlistsrch.FilterList = <?php echo $discount_report_view_list->GetFilterList() ?>;
fdiscount_report_viewlistsrch.Init();
fdiscount_report_viewlist.Init();
</script>
<?php } ?>
<?php
$discount_report_view_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($discount_report_view->Export == "") { ?>
...
<?php
$discount_report_view_list->Page_Terminate();
?>
