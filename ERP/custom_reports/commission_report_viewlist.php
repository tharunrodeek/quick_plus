...
<?php include_once "commission_report_viewinfo.php" ?>
...
$commission_report_view_list = NULL; // Initialize page object first

class ccommission_report_view_list extends ccommission_report_view {
	...
	// Table name
	var $TableName = 'commission_report_view';

	// Page object name
	var $PageObjName = 'commission_report_view_list';

	// Grid form hidden field names
	var $FormName = 'fcommission_report_viewlist';
	...
	function __construct() {
		...
		// Table object (commission_report_view)
		if (!isset($GLOBALS["commission_report_view"]) || get_class($GLOBALS["commission_report_view"]) == "ccommission_report_view") {
			$GLOBALS["commission_report_view"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["commission_report_view"];
		}
		...
		$this->AddUrl = "commission_report_viewadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "commission_report_viewdelete.php";
		$this->MultiUpdateUrl = "commission_report_viewupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'commission_report_view', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fcommission_report_viewlistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $commission_report_view;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($commission_report_view);
				$doc->Text = $sContent;
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fcommission_report_viewlistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fcommission_report_viewlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fcommission_report_viewlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fcommission_report_viewlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcommission_report_viewlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_commission_report_view\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_commission_report_view',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fcommission_report_viewlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($commission_report_view_list)) $commission_report_view_list = new ccommission_report_view_list();

// Page init
$commission_report_view_list->Page_Init();

// Page main
$commission_report_view_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$commission_report_view_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($commission_report_view->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = fcommission_report_viewlist = new ew_Form("fcommission_report_viewlist", "list");
fcommission_report_viewlist.FormKeyCountName = '<?php echo $commission_report_view_list->FormKeyCountName ?>';

// Form_CustomValidate event
fcommission_report_viewlist.Form_CustomValidate = 
...
fcommission_report_viewlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcommission_report_viewlist.Lists["x_customer_name[]"] = {"LinkField":"x_name","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_debtors_master"};
fcommission_report_viewlist.Lists["x_customer_name[]"].Data = "<?php echo $commission_report_view_list->customer_name->LookupFilterQuery(FALSE, "list") ?>";
fcommission_report_viewlist.Lists["x_created_employee[]"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_id","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_users"};
fcommission_report_viewlist.Lists["x_created_employee[]"].Data = "<?php echo $commission_report_view_list->created_employee->LookupFilterQuery(FALSE, "list") ?>";
fcommission_report_viewlist.Lists["x_stock_id"] = {"LinkField":"x_stock_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","x_long_description","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_stock_master"};
fcommission_report_viewlist.Lists["x_stock_id"].Data = "<?php echo $commission_report_view_list->stock_id->LookupFilterQuery(FALSE, "list") ?>";

// Form object for search
var CurrentSearchForm = fcommission_report_viewlistsrch = new ew_Form("fcommission_report_viewlistsrch");

// Validate function for search
fcommission_report_viewlistsrch.Validate = function(fobj) {
	...
	if (elm && !ew_CheckDateDef(elm.value))
		return this.OnError(elm, "<?php echo ew_JsEncode2($commission_report_view->transaction_date->FldErrMsg()) ?>");
...
// Form_CustomValidate event
fcommission_report_viewlistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
fcommission_report_viewlistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcommission_report_viewlistsrch.Lists["x_customer_name[]"] = {"LinkField":"x_name","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_debtors_master"};
fcommission_report_viewlistsrch.Lists["x_customer_name[]"].Data = "<?php echo $commission_report_view_list->customer_name->LookupFilterQuery(FALSE, "extbs") ?>";
fcommission_report_viewlistsrch.Lists["x_created_employee[]"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_id","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_users"};
fcommission_report_viewlistsrch.Lists["x_created_employee[]"].Data = "<?php echo $commission_report_view_list->created_employee->LookupFilterQuery(FALSE, "extbs") ?>";
fcommission_report_viewlistsrch.Lists["x_stock_id"] = {"LinkField":"x_stock_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","x_long_description","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_0_stock_master"};
fcommission_report_viewlistsrch.Lists["x_stock_id"].Data = "<?php echo $commission_report_view_list->stock_id->LookupFilterQuery(FALSE, "extbs") ?>";
</script>
<script type="text/javascript">
...
<?php if ($commission_report_view->Export == "") { ?>
<div class="ewToolbar">
<?php if ($commission_report_view_list->TotalRecs > 0 && $commission_report_view_list->ExportOptions->Visible()) { ?>
<?php $commission_report_view_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($commission_report_view_list->SearchOptions->Visible()) { ?>
<?php $commission_report_view_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($commission_report_view_list->FilterOptions->Visible()) { ?>
<?php $commission_report_view_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $commission_report_view_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($commission_report_view_list->TotalRecs <= 0)
			$commission_report_view_list->TotalRecs = $commission_report_view->ListRecordCount();
	} else {
		if (!$commission_report_view_list->Recordset && ($commission_report_view_list->Recordset = $commission_report_view_list->LoadRecordset()))
			$commission_report_view_list->TotalRecs = $commission_report_view_list->Recordset->RecordCount();
	}
	$commission_report_view_list->StartRec = 1;
	if ($commission_report_view_list->DisplayRecs <= 0 || ($commission_report_view->Export <> "" && $commission_report_view->ExportAll)) // Display all records
		$commission_report_view_list->DisplayRecs = $commission_report_view_list->TotalRecs;
	if (!($commission_report_view->Export <> "" && $commission_report_view->ExportAll))
		$commission_report_view_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$commission_report_view_list->Recordset = $commission_report_view_list->LoadRecordset($commission_report_view_list->StartRec-1, $commission_report_view_list->DisplayRecs);

	// Set no record found message
	if ($commission_report_view->CurrentAction == "" && $commission_report_view_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$commission_report_view_list->setWarningMessage(ew_DeniedMsg());
		if ($commission_report_view_list->SearchWhere == "0=101")
			$commission_report_view_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$commission_report_view_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$commission_report_view_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($commission_report_view->Export == "" && $commission_report_view->CurrentAction == "") { ?>
<form name="fcommission_report_viewlistsrch" id="fcommission_report_viewlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($commission_report_view_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fcommission_report_viewlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="commission_report_view">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$commission_report_view_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$commission_report_view->RowType = EW_ROWTYPE_SEARCH;

// Render row
$commission_report_view->ResetAttrs();
$commission_report_view_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($commission_report_view->invoice_no->Visible) { // invoice_no ?>
	<div id="xsc_invoice_no" class="ewCell form-group">
		<label for="x_invoice_no" class="ewSearchCaption ewLabel"><?php echo $commission_report_view->invoice_no->FldCaption() ?></label>
...
<input type="text" data-table="commission_report_view" data-field="x_invoice_no" name="x_invoice_no" id="x_invoice_no" size="30" maxlength="60" placeholder="<?php echo ew_HtmlEncode($commission_report_view->invoice_no->getPlaceHolder()) ?>" value="<?php echo $commission_report_view->invoice_no->EditValue ?>"<?php echo $commission_report_view->invoice_no->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_2" class="ewRow">
<?php if ($commission_report_view->transaction_date->Visible) { // transaction_date ?>
	<div id="xsc_transaction_date" class="ewCell form-group">
		<label for="x_transaction_date" class="ewSearchCaption ewLabel"><?php echo $commission_report_view->transaction_date->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<input type="text" data-table="commission_report_view" data-field="x_transaction_date" data-format="2" name="x_transaction_date" id="x_transaction_date" placeholder="<?php echo ew_HtmlEncode($commission_report_view->transaction_date->getPlaceHolder()) ?>" value="<?php echo $commission_report_view->transaction_date->EditValue ?>"<?php echo $commission_report_view->transaction_date->EditAttributes() ?>>
<?php if (!$commission_report_view->transaction_date->ReadOnly && !$commission_report_view->transaction_date->Disabled && !isset($commission_report_view->transaction_date->EditAttrs["readonly"]) && !isset($commission_report_view->transaction_date->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("fcommission_report_viewlistsrch", "x_transaction_date", {"ignoreReadonly":true,"useCurrent":false,"format":2});
</script>
<?php } ?>
</span>
...
<input type="text" data-table="commission_report_view" data-field="x_transaction_date" data-format="2" name="y_transaction_date" id="y_transaction_date" placeholder="<?php echo ew_HtmlEncode($commission_report_view->transaction_date->getPlaceHolder()) ?>" value="<?php echo $commission_report_view->transaction_date->EditValue2 ?>"<?php echo $commission_report_view->transaction_date->EditAttributes() ?>>
<?php if (!$commission_report_view->transaction_date->ReadOnly && !$commission_report_view->transaction_date->Disabled && !isset($commission_report_view->transaction_date->EditAttrs["readonly"]) && !isset($commission_report_view->transaction_date->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("fcommission_report_viewlistsrch", "y_transaction_date", {"ignoreReadonly":true,"useCurrent":false,"format":2});
</script>
<?php } ?>
</span>
	</div>
...
<?php if ($commission_report_view->customer_name->Visible) { // customer_name ?>
	<div id="xsc_customer_name" class="ewCell form-group">
		<label for="x_customer_name" class="ewSearchCaption ewLabel"><?php echo $commission_report_view->customer_name->FldCaption() ?></label>
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_customer_name"><?php echo (strval($commission_report_view->customer_name->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $commission_report_view->customer_name->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($commission_report_view->customer_name->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_customer_name[]',m:1,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($commission_report_view->customer_name->ReadOnly || $commission_report_view->customer_name->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="commission_report_view" data-field="x_customer_name" data-multiple="1" data-lookup="1" data-value-separator="<?php echo $commission_report_view->customer_name->DisplayValueSeparatorAttribute() ?>" name="x_customer_name[]" id="x_customer_name[]" value="<?php echo $commission_report_view->customer_name->AdvancedSearch->SearchValue ?>"<?php echo $commission_report_view->customer_name->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_4" class="ewRow">
<?php if ($commission_report_view->created_employee->Visible) { // created_employee ?>
	<div id="xsc_created_employee" class="ewCell form-group">
		<label for="x_created_employee" class="ewSearchCaption ewLabel"><?php echo $commission_report_view->created_employee->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_created_employee"><?php echo (strval($commission_report_view->created_employee->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $commission_report_view->created_employee->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($commission_report_view->created_employee->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_created_employee[]',m:1,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($commission_report_view->created_employee->ReadOnly || $commission_report_view->created_employee->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="commission_report_view" data-field="x_created_employee" data-multiple="1" data-lookup="1" data-value-separator="<?php echo $commission_report_view->created_employee->DisplayValueSeparatorAttribute() ?>" name="x_created_employee[]" id="x_created_employee[]" value="<?php echo $commission_report_view->created_employee->AdvancedSearch->SearchValue ?>"<?php echo $commission_report_view->created_employee->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_5" class="ewRow">
<?php if ($commission_report_view->stock_id->Visible) { // stock_id ?>
	<div id="xsc_stock_id" class="ewCell form-group">
		<label for="x_stock_id" class="ewSearchCaption ewLabel"><?php echo $commission_report_view->stock_id->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_stock_id"><?php echo (strval($commission_report_view->stock_id->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $commission_report_view->stock_id->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($commission_report_view->stock_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_stock_id',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($commission_report_view->stock_id->ReadOnly || $commission_report_view->stock_id->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="commission_report_view" data-field="x_stock_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $commission_report_view->stock_id->DisplayValueSeparatorAttribute() ?>" name="x_stock_id" id="x_stock_id" value="<?php echo $commission_report_view->stock_id->AdvancedSearch->SearchValue ?>"<?php echo $commission_report_view->stock_id->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_6" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($commission_report_view_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($commission_report_view_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $commission_report_view_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($commission_report_view_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($commission_report_view_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($commission_report_view_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($commission_report_view_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $commission_report_view_list->ShowPageHeader(); ?>
<?php
$commission_report_view_list->ShowMessage();
?>
<?php if ($commission_report_view_list->TotalRecs > 0 || $commission_report_view->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($commission_report_view_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> commission_report_view">
<form name="fcommission_report_viewlist" id="fcommission_report_viewlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($commission_report_view_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $commission_report_view_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="commission_report_view">
<div id="gmp_commission_report_view" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($commission_report_view_list->TotalRecs > 0 || $commission_report_view->CurrentAction == "gridedit") { ?>
<table id="tbl_commission_report_viewlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$commission_report_view_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$commission_report_view_list->RenderListOptions();

// Render list options (header, left)
$commission_report_view_list->ListOptions->Render("header", "left");
?>
<?php if ($commission_report_view->invoice_no->Visible) { // invoice_no ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->invoice_no) == "") { ?>
		<th data-name="invoice_no" class="<?php echo $commission_report_view->invoice_no->HeaderCellClass() ?>"><div id="elh_commission_report_view_invoice_no" class="commission_report_view_invoice_no"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->invoice_no->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="invoice_no" class="<?php echo $commission_report_view->invoice_no->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->invoice_no) ?>',1);"><div id="elh_commission_report_view_invoice_no" class="commission_report_view_invoice_no">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->invoice_no->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->invoice_no->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->invoice_no->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($commission_report_view->transaction_date->Visible) { // transaction_date ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->transaction_date) == "") { ?>
		<th data-name="transaction_date" class="<?php echo $commission_report_view->transaction_date->HeaderCellClass() ?>"><div id="elh_commission_report_view_transaction_date" class="commission_report_view_transaction_date"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->transaction_date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="transaction_date" class="<?php echo $commission_report_view->transaction_date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->transaction_date) ?>',1);"><div id="elh_commission_report_view_transaction_date" class="commission_report_view_transaction_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->transaction_date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->transaction_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->transaction_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($commission_report_view->description->Visible) { // description ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->description) == "") { ?>
		<th data-name="description" class="<?php echo $commission_report_view->description->HeaderCellClass() ?>"><div id="elh_commission_report_view_description" class="commission_report_view_description"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->description->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="description" class="<?php echo $commission_report_view->description->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->description) ?>',1);"><div id="elh_commission_report_view_description" class="commission_report_view_description">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->description->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->description->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->description->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($commission_report_view->customer_name->Visible) { // customer_name ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->customer_name) == "") { ?>
		<th data-name="customer_name" class="<?php echo $commission_report_view->customer_name->HeaderCellClass() ?>"><div id="elh_commission_report_view_customer_name" class="commission_report_view_customer_name"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->customer_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="customer_name" class="<?php echo $commission_report_view->customer_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->customer_name) ?>',1);"><div id="elh_commission_report_view_customer_name" class="commission_report_view_customer_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->customer_name->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->customer_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->customer_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($commission_report_view->reference_customer->Visible) { // reference_customer ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->reference_customer) == "") { ?>
		<th data-name="reference_customer" class="<?php echo $commission_report_view->reference_customer->HeaderCellClass() ?>"><div id="elh_commission_report_view_reference_customer" class="commission_report_view_reference_customer"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->reference_customer->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="reference_customer" class="<?php echo $commission_report_view->reference_customer->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->reference_customer) ?>',1);"><div id="elh_commission_report_view_reference_customer" class="commission_report_view_reference_customer">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->reference_customer->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->reference_customer->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->reference_customer->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($commission_report_view->created_employee->Visible) { // created_employee ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->created_employee) == "") { ?>
		<th data-name="created_employee" class="<?php echo $commission_report_view->created_employee->HeaderCellClass() ?>"><div id="elh_commission_report_view_created_employee" class="commission_report_view_created_employee"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->created_employee->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="created_employee" class="<?php echo $commission_report_view->created_employee->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->created_employee) ?>',1);"><div id="elh_commission_report_view_created_employee" class="commission_report_view_created_employee">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->created_employee->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->created_employee->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->created_employee->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($commission_report_view->user_commission->Visible) { // user_commission ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->user_commission) == "") { ?>
		<th data-name="user_commission" class="<?php echo $commission_report_view->user_commission->HeaderCellClass() ?>"><div id="elh_commission_report_view_user_commission" class="commission_report_view_user_commission"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->user_commission->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="user_commission" class="<?php echo $commission_report_view->user_commission->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->user_commission) ?>',1);"><div id="elh_commission_report_view_user_commission" class="commission_report_view_user_commission">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->user_commission->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->user_commission->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->user_commission->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($commission_report_view->quantity->Visible) { // quantity ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->quantity) == "") { ?>
		<th data-name="quantity" class="<?php echo $commission_report_view->quantity->HeaderCellClass() ?>"><div id="elh_commission_report_view_quantity" class="commission_report_view_quantity"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->quantity->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="quantity" class="<?php echo $commission_report_view->quantity->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->quantity) ?>',1);"><div id="elh_commission_report_view_quantity" class="commission_report_view_quantity">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->quantity->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->quantity->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->quantity->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($commission_report_view->total_commission->Visible) { // total_commission ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->total_commission) == "") { ?>
		<th data-name="total_commission" class="<?php echo $commission_report_view->total_commission->HeaderCellClass() ?>"><div id="elh_commission_report_view_total_commission" class="commission_report_view_total_commission"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->total_commission->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="total_commission" class="<?php echo $commission_report_view->total_commission->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->total_commission) ?>',1);"><div id="elh_commission_report_view_total_commission" class="commission_report_view_total_commission">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->total_commission->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->total_commission->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->total_commission->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($commission_report_view->stock_id->Visible) { // stock_id ?>
	<?php if ($commission_report_view->SortUrl($commission_report_view->stock_id) == "") { ?>
		<th data-name="stock_id" class="<?php echo $commission_report_view->stock_id->HeaderCellClass() ?>"><div id="elh_commission_report_view_stock_id" class="commission_report_view_stock_id"><div class="ewTableHeaderCaption"><?php echo $commission_report_view->stock_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="stock_id" class="<?php echo $commission_report_view->stock_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $commission_report_view->SortUrl($commission_report_view->stock_id) ?>',1);"><div id="elh_commission_report_view_stock_id" class="commission_report_view_stock_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $commission_report_view->stock_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($commission_report_view->stock_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($commission_report_view->stock_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$commission_report_view_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($commission_report_view->ExportAll && $commission_report_view->Export <> "") {
	$commission_report_view_list->StopRec = $commission_report_view_list->TotalRecs;
} else {

	// Set the last record to display
	if ($commission_report_view_list->TotalRecs > $commission_report_view_list->StartRec + $commission_report_view_list->DisplayRecs - 1)
		$commission_report_view_list->StopRec = $commission_report_view_list->StartRec + $commission_report_view_list->DisplayRecs - 1;
	else
		$commission_report_view_list->StopRec = $commission_report_view_list->TotalRecs;
}
$commission_report_view_list->RecCnt = $commission_report_view_list->StartRec - 1;
if ($commission_report_view_list->Recordset && !$commission_report_view_list->Recordset->EOF) {
	$commission_report_view_list->Recordset->MoveFirst();
	$bSelectLimit = $commission_report_view_list->UseSelectLimit;
	if (!$bSelectLimit && $commission_report_view_list->StartRec > 1)
		$commission_report_view_list->Recordset->Move($commission_report_view_list->StartRec - 1);
} elseif (!$commission_report_view->AllowAddDeleteRow && $commission_report_view_list->StopRec == 0) {
	$commission_report_view_list->StopRec = $commission_report_view->GridAddRowCount;
}

// Initialize aggregate
$commission_report_view->RowType = EW_ROWTYPE_AGGREGATEINIT;
$commission_report_view->ResetAttrs();
$commission_report_view_list->RenderRow();
while ($commission_report_view_list->RecCnt < $commission_report_view_list->StopRec) {
	$commission_report_view_list->RecCnt++;
	if (intval($commission_report_view_list->RecCnt) >= intval($commission_report_view_list->StartRec)) {
		$commission_report_view_list->RowCnt++;

		// Set up key count
		$commission_report_view_list->KeyCount = $commission_report_view_list->RowIndex;

		// Init row class and style
		$commission_report_view->ResetAttrs();
		$commission_report_view->CssClass = "";
		if ($commission_report_view->CurrentAction == "gridadd") {
		} else {
			$commission_report_view_list->LoadRowValues($commission_report_view_list->Recordset); // Load row values
		}
		$commission_report_view->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$commission_report_view->RowAttrs = array_merge($commission_report_view->RowAttrs, array('data-rowindex'=>$commission_report_view_list->RowCnt, 'id'=>'r' . $commission_report_view_list->RowCnt . '_commission_report_view', 'data-rowtype'=>$commission_report_view->RowType));
		...
		$commission_report_view_list->RenderListOptions();
?>
	<tr<?php echo $commission_report_view->RowAttributes() ?>>
<?php

// Render list options (body, left)
$commission_report_view_list->ListOptions->Render("body", "left", $commission_report_view_list->RowCnt);
?>
	<?php if ($commission_report_view->invoice_no->Visible) { // invoice_no ?>
		<td data-name="invoice_no"<?php echo $commission_report_view->invoice_no->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_invoice_no" class="commission_report_view_invoice_no">
<span<?php echo $commission_report_view->invoice_no->ViewAttributes() ?>>
<?php echo $commission_report_view->invoice_no->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($commission_report_view->transaction_date->Visible) { // transaction_date ?>
		<td data-name="transaction_date"<?php echo $commission_report_view->transaction_date->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_transaction_date" class="commission_report_view_transaction_date">
<span<?php echo $commission_report_view->transaction_date->ViewAttributes() ?>>
<?php echo $commission_report_view->transaction_date->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($commission_report_view->description->Visible) { // description ?>
		<td data-name="description"<?php echo $commission_report_view->description->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_description" class="commission_report_view_description">
<span<?php echo $commission_report_view->description->ViewAttributes() ?>>
<?php echo $commission_report_view->description->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($commission_report_view->customer_name->Visible) { // customer_name ?>
		<td data-name="customer_name"<?php echo $commission_report_view->customer_name->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_customer_name" class="commission_report_view_customer_name">
<span<?php echo $commission_report_view->customer_name->ViewAttributes() ?>>
<?php echo $commission_report_view->customer_name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($commission_report_view->reference_customer->Visible) { // reference_customer ?>
		<td data-name="reference_customer"<?php echo $commission_report_view->reference_customer->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_reference_customer" class="commission_report_view_reference_customer">
<span<?php echo $commission_report_view->reference_customer->ViewAttributes() ?>>
<?php echo $commission_report_view->reference_customer->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($commission_report_view->created_employee->Visible) { // created_employee ?>
		<td data-name="created_employee"<?php echo $commission_report_view->created_employee->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_created_employee" class="commission_report_view_created_employee">
<span<?php echo $commission_report_view->created_employee->ViewAttributes() ?>>
<?php echo $commission_report_view->created_employee->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($commission_report_view->user_commission->Visible) { // user_commission ?>
		<td data-name="user_commission"<?php echo $commission_report_view->user_commission->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_user_commission" class="commission_report_view_user_commission">
<span<?php echo $commission_report_view->user_commission->ViewAttributes() ?>>
<?php echo $commission_report_view->user_commission->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($commission_report_view->quantity->Visible) { // quantity ?>
		<td data-name="quantity"<?php echo $commission_report_view->quantity->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_quantity" class="commission_report_view_quantity">
<span<?php echo $commission_report_view->quantity->ViewAttributes() ?>>
<?php echo $commission_report_view->quantity->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($commission_report_view->total_commission->Visible) { // total_commission ?>
		<td data-name="total_commission"<?php echo $commission_report_view->total_commission->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_total_commission" class="commission_report_view_total_commission">
<span<?php echo $commission_report_view->total_commission->ViewAttributes() ?>>
<?php echo $commission_report_view->total_commission->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($commission_report_view->stock_id->Visible) { // stock_id ?>
		<td data-name="stock_id"<?php echo $commission_report_view->stock_id->CellAttributes() ?>>
<span id="el<?php echo $commission_report_view_list->RowCnt ?>_commission_report_view_stock_id" class="commission_report_view_stock_id">
<span<?php echo $commission_report_view->stock_id->ViewAttributes() ?>>
<?php echo $commission_report_view->stock_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$commission_report_view_list->ListOptions->Render("body", "right", $commission_report_view_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($commission_report_view->CurrentAction <> "gridadd")
		$commission_report_view_list->Recordset->MoveNext();
}
?>
</tbody>
<?php
...
$commission_report_view->RowType = EW_ROWTYPE_AGGREGATE;
$commission_report_view->ResetAttrs();
$commission_report_view_list->RenderRow();
?>
<?php if ($commission_report_view_list->TotalRecs > 0 && ($commission_report_view->CurrentAction <> "gridadd" && $commission_report_view->CurrentAction <> "gridedit")) { ?>
<tfoot><!-- Table footer -->
...
$commission_report_view_list->ListOptions->Render("footer", "left");
?>
	<?php if ($commission_report_view->invoice_no->Visible) { // invoice_no ?>
		<td data-name="invoice_no" class="<?php echo $commission_report_view->invoice_no->FooterCellClass() ?>"><span id="elf_commission_report_view_invoice_no" class="commission_report_view_invoice_no">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($commission_report_view->transaction_date->Visible) { // transaction_date ?>
		<td data-name="transaction_date" class="<?php echo $commission_report_view->transaction_date->FooterCellClass() ?>"><span id="elf_commission_report_view_transaction_date" class="commission_report_view_transaction_date">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($commission_report_view->description->Visible) { // description ?>
		<td data-name="description" class="<?php echo $commission_report_view->description->FooterCellClass() ?>"><span id="elf_commission_report_view_description" class="commission_report_view_description">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($commission_report_view->customer_name->Visible) { // customer_name ?>
		<td data-name="customer_name" class="<?php echo $commission_report_view->customer_name->FooterCellClass() ?>"><span id="elf_commission_report_view_customer_name" class="commission_report_view_customer_name">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($commission_report_view->reference_customer->Visible) { // reference_customer ?>
		<td data-name="reference_customer" class="<?php echo $commission_report_view->reference_customer->FooterCellClass() ?>"><span id="elf_commission_report_view_reference_customer" class="commission_report_view_reference_customer">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($commission_report_view->created_employee->Visible) { // created_employee ?>
		<td data-name="created_employee" class="<?php echo $commission_report_view->created_employee->FooterCellClass() ?>"><span id="elf_commission_report_view_created_employee" class="commission_report_view_created_employee">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($commission_report_view->user_commission->Visible) { // user_commission ?>
		<td data-name="user_commission" class="<?php echo $commission_report_view->user_commission->FooterCellClass() ?>"><span id="elf_commission_report_view_user_commission" class="commission_report_view_user_commission">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($commission_report_view->quantity->Visible) { // quantity ?>
		<td data-name="quantity" class="<?php echo $commission_report_view->quantity->FooterCellClass() ?>"><span id="elf_commission_report_view_quantity" class="commission_report_view_quantity">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($commission_report_view->total_commission->Visible) { // total_commission ?>
		<td data-name="total_commission" class="<?php echo $commission_report_view->total_commission->FooterCellClass() ?>"><span id="elf_commission_report_view_total_commission" class="commission_report_view_total_commission">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span><span class="ewAggregateValue">
<?php echo $commission_report_view->total_commission->ViewValue ?></span>
		</span></td>
	<?php } ?>
	<?php if ($commission_report_view->stock_id->Visible) { // stock_id ?>
		<td data-name="stock_id" class="<?php echo $commission_report_view->stock_id->FooterCellClass() ?>"><span id="elf_commission_report_view_stock_id" class="commission_report_view_stock_id">
		&nbsp;
		</span></td>
	<?php } ?>
<?php
...
$commission_report_view_list->ListOptions->Render("footer", "right");
?>
	</tr>
</tfoot>
<?php } ?>
...
<?php } ?>
<?php if ($commission_report_view->CurrentAction == "") { ?>
...
// Close recordset
if ($commission_report_view_list->Recordset)
	$commission_report_view_list->Recordset->Close();
?>
<?php if ($commission_report_view->Export == "") { ?>
<div class="box-footer ewGridLowerPanel">
<?php if ($commission_report_view->CurrentAction <> "gridadd" && $commission_report_view->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($commission_report_view_list->Pager)) $commission_report_view_list->Pager = new cPrevNextPager($commission_report_view_list->StartRec, $commission_report_view_list->DisplayRecs, $commission_report_view_list->TotalRecs, $commission_report_view_list->AutoHidePager) ?>
<?php if ($commission_report_view_list->Pager->RecordCount > 0 && $commission_report_view_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($commission_report_view_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $commission_report_view_list->PageUrl() ?>start=<?php echo $commission_report_view_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($commission_report_view_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $commission_report_view_list->PageUrl() ?>start=<?php echo $commission_report_view_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $commission_report_view_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($commission_report_view_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $commission_report_view_list->PageUrl() ?>start=<?php echo $commission_report_view_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($commission_report_view_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $commission_report_view_list->PageUrl() ?>start=<?php echo $commission_report_view_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $commission_report_view_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($commission_report_view_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $commission_report_view_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $commission_report_view_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $commission_report_view_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($commission_report_view_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($commission_report_view_list->TotalRecs == 0 && $commission_report_view->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($commission_report_view_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($commission_report_view->Export == "") { ?>
<script type="text/javascript">
fcommission_report_viewlistsrch.FilterList = <?php echo $commission_report_view_list->GetFilterList() ?>;
fcommission_report_viewlistsrch.Init();
fcommission_report_viewlist.Init();
</script>
<?php } ?>
<?php
$commission_report_view_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($commission_report_view->Export == "") { ?>
...
<?php
$commission_report_view_list->Page_Terminate();
?>
