...
page(trans($help_context = "Software Upgrade"), false, false, "", $js);
...
if (get_post('Upgrade')) 
{
    ...
    if ($comp === '')
		display_error(trans('Select company to be upgraded.'));
	else {
		...
		if ($patch)
		{
			if (!$patch->upgrade_company($comp, check_value('force')))
				...
				display_notification(trans("Company upgraded successfully."));
...
$th = array(trans("Company"), trans("Table set"), trans("Current version"), trans("Last log"), trans('Upgrade'));
...
foreach($site_status as $i => $comp)
{
	...
	if (file_exists($log))
	{
		label_cell(viewer_link(trans('View log'), "admin/view/view_upgrade_log.php?id=$i", null, $i, 'log.png')
		  .button('Clear'.$i, trans('Clear'), trans('Clear log'), ICON_DELETE), 'align=center');
		submit_js_confirm('Clear'.$i, trans("Do you really want to clear this upgrade log?"));
	} else
...
if ($uptodate)
	display_note(trans('All company database schemes are up to date.'));
else {
	if (get_post('select_comp') === '')
		display_note(trans("Select company for incremental upgrade."), 0, 1, "class='stockmankofg'");
	submit_center('Upgrade', trans('Upgrade'), true, trans('Save database and perform upgrade'), 'nonajax');
}
end_form();
