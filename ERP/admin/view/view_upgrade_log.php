...
page(trans($help_context = "Log View"), true);
...
if (!isset($_GET['id'])) 
{
	...
	display_note(trans("The script must be called with a valid company number."));
	end_page();
}

display_heading(sprintf(trans("Upgrade log for company '%s'"), $_GET['id']));
br();
  start_table();
	start_row();
