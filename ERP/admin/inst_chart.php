...
page(trans($help_context = "Install Charts of Accounts"), false, false, '', $js);
...
if ($id = find_submit('Delete', false))
{
	...
	if (($extensions[$id]['type']=='chart') && uninstall_package($extensions[$id]['package'])) {
		...
		if (update_extensions($extensions)) {
			display_notification(trans("Selected chart has been successfully deleted"));
...
start_form(true);
	...
	if (!$mods)
		display_note(trans("No optional chart of accounts is currently available."));
	else
	{
        uasort($mods, 'sortByOption');

		$th = array(trans("Chart"),  trans("Installed"), trans("Available"), trans("Encoding"), "", "");
		$k = 0;
		...
		foreach($mods as $pkg_name => $ext)
		{
			...
			label_cell($encoding ? $encoding : trans("Unknown"));
