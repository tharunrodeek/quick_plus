...
page(trans($help_context = "Install/Activate extensions"), false, false, "", $js);
...
function handle_delete($id)
{
	...
	if (update_extensions($extensions)) {
		display_notification(trans("Selected extension has been successfully deleted"));
	}
	return true;
}
//
...
function display_extensions($mods)
{
	...
	$th = array(trans("Extension"),trans("Modules provided"), trans("Options provided"),
		 trans("Installed"), trans("Available"),  "", "");
	table_header($th);
	...
	foreach($mods as $pkg_name => $ext)
	{
		...
		label_cell($id === null ? trans("None") :
			(($installed && ($installed != '-' || $installed != '')) ? $installed : trans("Unknown")));
		label_cell($available ? $available : trans("Unknown"));

		if (!$available && $ext['type'] == 'extension')	{// third-party plugin
			if (!$installed)
				button_cell('Local'.$ext['package'], trans("Install"), trans('Install third-party extension.'),
					ICON_DOWN);
			else
				label_cell('');
			...
			button_cell('Update'.$pkg_name, $installed ? trans("Update") : trans("Install"),
				trans('Upload and install latest extension package'), ICON_DOWN);
		else
			label_cell('');
	...
	submit_center_first('Refresh', trans("Update"), '', null);
...
function company_extensions($id)
{
	...
	$th = array(trans("Extension"),trans("Modules provided"), trans("Options provided"), trans("Active"));
	...
	end_table(1);
	submit_center('Refresh', trans('Update'), true, false, 'default');
}
...
if (get_post('Refresh')) {
	...
	foreach($exts as $i => $ext) {
		if ($ext['package'] && ($ext['active'] ^ check_value('Active'.$i))) 
		{
			...
			{
				display_warning(sprintf(trans("Package '%s' is incompatible with current application version and cannot be activated.\n")
					. trans("Check Install/Activate page for newer package version."), $ext['name']));
				continue;
			}
	...
	if(!$result) {
		display_error(trans('Status change for some extensions failed.'));
		$Ajax->activate('ext_tbl'); // refresh settings display
	}else
		display_notification(trans('Current active extensions set has been saved.'));
}
...
if ($set == -1) 
{
	...
	if (!$mods)
		display_note(trans("No optional extension module is currently available."));
	else
