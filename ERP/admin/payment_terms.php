...
page(trans($help_context = "Payment Terms"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error( trans("The number of days or the day in the following month must be numeric."));
		set_focus('DayNumber');
	} 
	elseif (strlen($_POST['terms']) == 0) 
	{
		$input_error = 1;
		display_error( trans("The Terms description must be entered."));
		set_focus('terms');
	}
	...
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1) 
    	{
    		update_payment_terms($selected_id, $from_now, $_POST['terms'], $days); 
 			$note = trans('Selected payment terms have been updated');
    	} 
    	else 
    	{
			add_payment_terms($from_now, $_POST['terms'], $days);
			$note = trans('New payment terms have been added');
    	}
...
if ($Mode == 'Delete')
{
	...
	{
		display_error(trans("Cannot delete this payment term, because customer accounts have been created referring to this term."));
	} 
	else 
	{
		if (key_in_foreign_table($selected_id, 'suppliers', 'payment_terms'))
		{
			display_error(trans("Cannot delete this payment term, because supplier accounts have been created referring to this term"));
		} 
		else 
		{
			//only delete if used in neither customer or supplier accounts
			delete_payment_terms($selected_id);
			display_notification(trans('Selected payment terms have been deleted'));
		}
	}
...
$th = array(trans("Description"), trans("Type"), trans("Due After/Days"), "", "");
...
while ($myrow = db_fetch($result)) 
{
    ...
    label_cell($type == PTT_DAYS ? "$days ".trans("days") : ($type == PTT_FOLLOWING ? $days : trans("N/A")));
	inactive_control_cell($myrow["terms_indicator"], $myrow["inactive"], 'payment_terms', "terms_indicator");
 	edit_button_cell("Edit".$myrow["terms_indicator"], trans("Edit"));
 	delete_button_cell("Delete".$myrow["terms_indicator"], trans("Delete"));
    end_row();
...
text_row(trans("Terms Description:"), 'terms', null, 40, 40);

payment_type_list_row(trans("Payment type:"), 'type', null, true);

if ( in_array(get_post('type'), array(PTT_FOLLOWING, PTT_DAYS))) 
	text_row_ex(trans("Days (Or Day In Following Month):"), 'DayNumber', 3);
else
	hidden('DayNumber', 0);
