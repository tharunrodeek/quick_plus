...
page(trans($help_context = "Printing Profiles"));
...
function get_reports() {
	...
	// to save time, store in session.
		...
		$reports = array( '' => trans('Default printing destination'));
...
if ( get_post('submit'))
{
	...
	{
		$error = 1;
		display_error( trans("Printing profile name cannot be empty."));
		set_focus('name');
	} 
	...
	if (!$error)
	{
		...
		if ($selected_id == '') {
			display_notification_centered(trans('New printing profile has been created'));
			clear_form($selected_id);
		} else {
			display_notification_centered(trans('Printing profile has been updated'));
		}
	}
}
...
if(get_post('delete'))
{
 	if (!check_delete(get_post('name'))) {
		...
		display_notification(trans('Selected printing profile has been deleted'));
		clear_form();
 	}
}
...
start_form();
start_table();
print_profiles_list_row(trans('Select printing profile'). ':', 'profile_id', null,
	trans('New printing profile'), true);
end_table();
echo '<hr>';
start_table();
if (get_post('profile_id') == '')
	text_row(trans("Printing Profile Name").':', 'name', null, 30, 30);
else
	label_cells(trans("Printing Profile Name").':', get_post('profile_id'));
end_table(1);
...
$th = array(trans("Report Id"), trans("Description"), trans("Printer"));
table_header($th);
...
foreach(get_reports() as $rep => $descr)
{
    ...
    label_cell($descr == '' ? '???<sup>1)</sup>' : trans($descr));
	$_POST['Prn'.$rep] = isset($prints[$rep]) ? $prints[$rep] : '';
    echo '<td>';
	echo printers_list('Prn'.$rep, null, 
		$rep == '' ? trans('Browser support') : trans('Default'));
	echo '</td>';
	if ($descr == '') $unkn = 1;
    end_row();
}
end_table();
if ($unkn)
	display_note('<sup>1)</sup>&nbsp;-&nbsp;'.trans("no title was found in this report definition file."), 0, 1, '');
else
	echo '<br>';

div_start('controls');
if (get_post('profile_id') == '') {
	submit_center('submit', trans("Add New Profile"), true, '', 'default');
} else {
	submit_center_first('submit', trans("Update Profile"),
	  trans('Update printer profile'), 'default');
	submit_center_last('delete', trans("Delete Profile"),
	  trans('Delete printer profile (only if not used by any user)'), true);
}
div_end();
