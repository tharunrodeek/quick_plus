...
class hooks_import_paypal extends hooks {
	...
	*/
	function install_options($app) {
		...
		switch($app->id) {
			case 'GL':
				$app->add_lapp_function(0, trans('Import Paypal Transactions'), 
				$path_to_root.'/modules/import_paypal/import_paypal.php', 'SA_PAYPALIMPORT', MENU_TRANSACTION);
				break;
			case 'system':
				$app->add_lapp_function(1, trans('Paypal Import Setup'), 
	...
	{
        $security_sections[SS_IMPORTPAYPALITEMS] =  trans("Import Paypal Items");
        
        $security_areas['SA_PAYPALIMPORT'] = array(SS_IMPORTPAYPALITEMS|107, trans("Import Paypal Items"));
        $security_areas['SA_PAYPALSETUP'] = array(SS_IMPORTPAYPALITEMS|108, trans("Setup Paypal Import"));
