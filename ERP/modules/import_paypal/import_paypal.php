...
page(trans("Import of Paypal transactions"));
...
if (isset($_POST['import_paypal'])) {
  if (isset($_FILES['imp']) && $_FILES['imp']['name'] != '') {
      ...
      if (!$fp)
      	die(trans("can not open file")." ".$saveFilename);
  ...
  } else display_error(trans("No CSV file selected"));
}
...
label_row(trans("CSV Import File:"), "<input type='file' id='imp' name='imp'>");
hidden("filename",$saveFilename);

end_table(1);

submit_center('import_paypal', trans("Import Paypal Transactions"));
