...
page(trans("Select payments' account"));
$lines = 0;

if (isset($_POST['import_paypal']) && $unconfirmedPayments != 0) {
  display_error(trans("You must select an account for every payment"));
} else {
  if ((isset($_POST['import_paypal']) && $unconfirmedPayments == 0 && !$postBack) || $totalPayments == 0) {
      if (isset($filename)) {
        ...
        log_message("Paypal act:".$paypal_bank_id.", withdrawal:".$paypal_withdraw_id.", sales:".$paypal_sales_act.", tax:".$paypal_sales_tax_act);
      	...
      	$fp = @fopen($filename, "r");
      	if (!$fp)
            die(trans("can not open file")." ".$filename);
          ...
          switch (strtoupper($type)) {
            ...
            case "WITHDRAW FUNDS TO A BANK ACCOUNT":
              ...
              add_bank_transfer($paypal_bank_id, $paypal_withdraw_id, $date,
                      -$gross, $ref, trans("Paypal transfer"), 0, 0);
              $transfers += 1;
              break;
          }
        ...
        unset($headings);
      	...
      	if ($new_custs + $updated_custs > 0)
      	     display_notification($new_custs." ".trans("customers created,")." ".$updated_custs." ".trans("customers updated"));
      	if ($invoices > 0) display_notification($invoices." ".trans("invoices added."));
        if ($receipts > 0) display_notification($receipts." ".trans("receipts added."));
        if ($payments > 0) display_notification($payments." ".trans("payments added."));
        if ($transfers > 0) display_notification($transfers." ".trans("withdrawals added."));
      }
    }
};
...
if ($lines > 0) {
  submit_center('ok', trans("OK"));
} else {
  $sql = get_paypal_items($user_id);

  $cols =
  array(
      trans("PayPal Reference") => 'ref',
      trans("Date") => 'date',
      trans("Person") => 'person',
      trans("Memo") => 'mem',
      trans("Amount") => 'amount',
      trans("Account") => array('fun'=>'select_account')
     );
