...
if(isset($_GET['requisitionid'])) {
	...
	{
		display_error(sprintf(trans("Requisition #%d is already completed or does not exists."), $_GET['requisitionid']));
		submenu_option(trans( "Entry &New Requisition."), '/modules/requisitions/requisitions.php');
		display_footer_exit();
	}
	if(isset($_GET['complete']) && $_GET['complete'] == 'yes') {
			complete_requisition($_GET['requisitionid']);
			display_notification(sprintf(trans("Requisition #%d has been completed."), $_GET['requisitionid']));
			submenu_option(trans( "Entry &New Requisition."), '/modules/requisitions/requisitions.php');
			display_footer_exit();
	}
	else
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(trans("The item of use cannot be empty."));
		set_focus('name');
	}
	if (strlen($_POST['estimate_price']) == 0) 
	{
		$input_error = 1;
		display_error(trans("The estimated price be empty."));
		set_focus('rate');
	}
	...
	if ($input_error != 1) 
	{
    	if ($selected_id != -1) 
    	{
			...
			display_notification(trans('Selected requisition details has been updated.'));
    	} 
    	else 
    	{
    		add_requisition_detail($_POST['requisitionid'], $_POST['item_code'], $_POST['purpose'], $_POST['order_quantity'], input_num('estimate_price'));
			display_notification(trans('New requisition details has been added'));
    	}
...
if ($Mode == 'Delete')
{
	...
	display_notification(trans('Selected requisition detail has been deleted'));
...
$th = array(trans("Point of use"), trans("Narrative"), trans("Application Date"), trans("Completation"));
...
while ($myrow = db_fetch($result)) 
{
	...
	echo "<td><a href='requisition_details.php?complete=yes&requisitionid=".$myrow['requisition_id']."'>".trans("Complete")."</a></td>\n";
...
$th = array(trans("Item Code"), trans("Item Name"), trans("Purpose"), trans("Qrder Quantity"), trans("Estimate Price"), "", "");
...
while ($myrow = db_fetch($result)) 
{
	...
	amount_cell($myrow["estimate_price"]);
 	...
 	edit_button_cell("Edit".$myrow['requisition_detail_id'], trans("Edit"));
 	delete_button_cell("Delete".$myrow['requisition_detail_id'], trans("Delete"));
...
sales_local_items_list_row(trans("Item :"), 'item_code', null, false, false);
text_row(trans("Purpose :"), 'purpose', null, 50, 50);

	$res = get_item_edit_info(get_post('item_code'));
	$dec =  $res["decimals"] == '' ? 0 : $res["decimals"];
	$units = $res["units"] == '' ? trans('kits') : $res["units"];

qty_row(trans("Requisition Quantity:"), 'order_quantity', number_format2(1, $dec), '', $units, $dec);
amount_row(trans("Estimate Price :"), 'estimate_price', null, null, null, 2);
