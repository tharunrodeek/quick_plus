...
function display_bank_trans($yr, $mo, &$dec, &$rep, $dimension, $dimension2, $date)
{
	...
	while ($account=db_fetch($result))
	{
		...
		$rep->Font();	
		$rep->NewLine(2);
		$rep->TextCol(0, 1,	trans("Opening Balance"), 0, 4, 0, $fill, NULL, 1);
		for ($i = 1; $i <= 14; $i++)
		{
			if (!$is_home)
		...
		$rep->NewLine(2);
		$rep->TextCol(0, 1,	trans("Net Cash Value"), 0, 4, 0, $fill, NULL, 1);
		for ($i = 1; $i <= 14; $i++)
		{
			if (!$is_home)
		...
		$fill = !$fill;
		$rep->NewLine(2);
		$rep->TextCol(0, 1,	trans("Closing Balance"), 0, 4, 0, $fill, NULL, 1);
		for ($i = 1; $i <= 14; $i++)
		{
	...
	$rep->Line($rep->row - 8);
	$rep->NewLine(2);
	$rep->TextCol(0, 1,	trans("Total Bank Accounts"), 0, 4, 0, $fill, NULL, 1);
	for ($i = 1; $i <= 14; $i++)
	{
...
function print_cash_flow_statement()
{
	...
	if ($comments)
		$comments .= ". - ";
	$comments .= trans("Balances in Home Currency");
   	$title = trans("Cash Flow Statement");
	...
	$header_row[] = trans("Fiscal YTD");
	$header_row[] = trans("12 Mo. to Date");
	for ($i = 12; $i < 14; $i++)
	{
    ...
    else
    {
   	...
   	// Footer Settings
   	$footerenable = true;
   	$footertext = trans('For Management Purposes Only');
