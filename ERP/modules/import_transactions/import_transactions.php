...
if ((isset($_POST['type']))) 
{
 ...
 if (isset($_FILES['imp']) && $_FILES['imp']['name'] != '')
 {
   ...
   if (!$fp)
   {
    display_error(trans("Error opening file $filename"));
   }
   else 
   {
     begin_transaction();
     ...
     while ($data = fgetcsv($fp, 4096, $sep))
     {
       if (($line++ == 0) && ($skippedheader == false))
           {display_notification_centered(trans("Skipped header. (line $line in import file '{$_FILES['imp']['name']}')"));$skippedheader = true;continue;}
           ...
           //All amounts to the right of amt are ignored since only considering payments which are to the left of deposits on a bank statement.     
       {
               ...
               if ((($ignore == "")||($ignore == null) || empty($ignore)) && ($amt > 0.01 )){} else 
                 {
                   display_notification_centered(trans("Ignoring deposit. Use same csv under deposit processing. (line $line in import file '{$_FILES['imp']['name']}')")); 
                   $error = false;
                   $prev_ref = $reference;
                   continue;                  
                 } 
      ...
      if (($type == ST_BANKDEPOSIT) && ($stateformat!=null))
      {
           //All amounts to the left of amt are ignored since only considering deposits which are to the left of payments on a bank statement.     
               ...
               if ((($ignore == "")||($ignore == null)|| empty($ignore) ) && ($amt > 0.01 )){} else 
                   {
                    display_notification_centered(trans("Ignoring payment. Use same csv under payment processing.(line $line in import file '{$_FILES['imp']['name']}')"));  
                    $error = false;
                    $prev_ref = $reference;
                    continue;
                   } 
       ...
       if (($type == ST_SALESORDER) || ($type == ST_SALESINVOICE))
        ...
        if (($type == ST_BANKDEPOSIT) || ($type == ST_BANKPAYMENT) || ($type == ST_JOURNAL))
        {
            ...
            if ($dim1_ref != '' && $dim1 == null) {display_error(trans("Error: Could not find dimension with dimension reference '$dim1_ref' (line $line in import file '{$_FILES['imp']['name']}')"));$error = true;}
            $dim2 = get_dimension_id_from_dimref($dim2_ref);
            if ($dim2_ref != '' && $dim2 == null) {display_error(trans("Error: Could not find dimension with dimension reference '$dim2_ref' (line $line in import file '{$_FILES['imp']['name']}')"));$error = true;}
        }

         if ($reference == '' ){display_error(trans("$line does not have a reference. (line $line in import file '{$_FILES['imp']['name']}')"));$error = true;}
         if (($Refs->exists($type, $reference)) && ($reference!=$prev_ref)){
          display_error(trans("Error: Reference from table 'refs': '$reference' is already in use. (line $line in import file '{$_FILES['imp']['name']}')"));$error = true;}   
      ...
      if (is_date($date)==false)
          {
	  	display_error(trans("Error: date '$date' not properly formatted (line $line in import file '{$_FILES['imp']['name']}')"));
		$error = true;
	  }
      //$date = sql2date($date);
     if ((is_date_in_fiscalyear($date)) == false) {display_error(trans("Error: Date not within company fiscal year. Make sure date is in dd/mm/yyyy format and your csv years are 4 digits long. Check that current fiscal year is active under Setup..Company Setup"));$error=true;}
     // validation for 
      ...
      if (!$error)
     {
       if (($type == ST_JOURNAL))
       {
         if (gl_account_in_bank_accounts($code_id) == true){display_notification_centered(trans("Error: Bank account detected in journal. No processing of bank accounts allowed. (line $line in import file '{$_FILES['imp']['name']}')"));$error = true;}
       ...
       elseif (($type == ST_BANKDEPOSIT || $type == ST_BANKPAYMENT) && ($amt > 0))
       {
         ...
         {
             display_notification_centered(trans("Warning: Taxtype used with Asset or Liability - $curEntryId, $date, $code_id.(line $line in import file '{$_FILES['imp']['name']}')"));
         } 
       }
       
       elseif (($type == ST_BANKDEPOSIT || $type == ST_BANKPAYMENT) && ($amt < 0))
       {
           display_notification_centered(trans("Error: Credit amounts represented by negative amounts being entered. Check csv file is correct.(line $line in import file '{$_FILES['imp']['name']}')"));
           $error = true;
       }
    ...
    if (($type == ST_BANKDEPOSIT) || ($type == ST_BANKPAYMENT) || ($type == ST_JOURNAL))
    {
        ...
        {
            display_notification_centered(trans("Error: Debits do not equal credits.")); 
            $errCnt = $errCnt + 1;
        }
    }
...
// Commit import to database
 ...
 if (!$trial) {
    if ($errCnt == 0) {
        if ($entryCount > 0) {
		    commit_transaction();
		    display_notification_centered(trans("$entryCount $typeString have been imported."));
	    } else display_error(trans("Import file contained no $typeString."));
    }
 } else {
	if ($errCnt==0) {
        if ($entryCount > 0) display_notification_centered(trans("$entryCount $typeString would have been successful if imported. Uncheck Trial check before importing."));
        else display_notification_centered(trans("Import file contained no $typeString. Populate file with data before importing."));
	}
	if (($errCnt>0) && $displayed_at_least_once) display_notification_centered(trans("$errCnt error(s) detected. Correct before importing."));
 }
 $errCnt =0;
