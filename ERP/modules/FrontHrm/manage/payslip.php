...
if (isset($_GET['ModifyPaySlip'])) {
	$_SESSION['page_title'] = sprintf(trans("Modifying Employee Payslip # %d."), $_GET['trans_no']);
	$help_context = "Modifying Employee Payslip";
}
else
	$_SESSION['page_title'] = trans($help_context = "Employee Payslip Entry");
...
if (isset($_GET['AddedID'])) {
    ...
    if($payslip_no) {
    	display_notification_centered( sprintf(trans("Payslip #%d has been entered"), $payslip_no));
        display_note(get_gl_view_str($trans_type, $trans_no, trans("&View this Transaction")));

	    reset_focus();
	    hyperlink_params($path_to_root.'/modules/FrontHrm/manage/pay_advice.php', trans('Make Payment &Advice for this Payslip'), 'PayslipNo='.$payslip_no);
	    hyperlink_params($_SERVER['PHP_SELF'], trans('Enter &New Payslip'), 'NewPayslip=Yes');
...
elseif (isset($_GET['UpdatedID'])) {
	...
	$trans_type = ST_JOURNAL;
   	...
   	display_notification_centered( trans('Employee Payslip has been updated #') . $trans_no);
    display_note(get_gl_view_str($trans_type, $trans_no, trans('&View this Journal Entry')));

   	hyperlink_no_params($path_to_root.'/gl/inquiry/payslip_inquiry.php', trans('Return to Payslip &Inquiry'));
...
function validate_payslip_generation() {
	...
	if (!$_POST['person_id']) {
		display_error(trans('Employee not selected'));
		set_focus('person_id');
		return false;
	} 
    ...
    if (date_comp($_POST['from_date'], Today()) > 0) {
		display_error(trans('Cannot pay for the date in the future.'));
		set_focus('from_date');
		return false;
	}
...
if (isset($_POST['Process'])) {
	...
	if($_SESSION['journal_items']->empty_payment) {
		display_error(trans('Employee cannot getting paid for non-working period.'));
		set_focus('from_date');
		$input_error = 1;
	}
	if ($input_error == 1)
...
if (!count($_SESSION['journal_items']->gl_items)) {
	br();
	submit_center('GeneratePayslip', trans("Generate Payslip"), trans('Generate Payslip For Process'), false);
	br();
}
...
if (count($_SESSION['journal_items']->gl_items)) {
	...
	submit_center_first('Process', trans('Process PaySlip'), trans('Process journal entry only if debits equal to credits'));
	submit_center_last('CancelOrder', trans('Cancel'), trans('Cancels document entry or removes Gl items'), true);
}
div_end();
