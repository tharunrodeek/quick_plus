...
page(trans($help_context = "Manage Salary Structure"), false, false, '', $js);
...
function can_process($selected_id) {
	...
	if (!$selected_id) {
		...
		display_error(trans("Select salary scale"));
	...
	foreach($_POST as $p=>$val) {
		...
		if(substr($p, 0, 7) == "Account") {
			...
			if(input_num("Debit".$val) && input_num("Credit".$val)) {
				display_error(trans("Only one amount(debit or credit) is allowed per rule"));
				set_focus("Debit".$val);
				return false;
			}
		}
...
function handle_submit(&$selected_id) {
	...
	if(empty($payroll_rules))
		display_notification(trans("No data entered"));
    else {
		...
		display_notification(trans("Salary structure has been updated."));
	}
...
if (isset($_POST['delete'])) {
	...
	delete_salary_structure($selected_id);
	display_notification(trans("Selected structure has been deleted."));
...
function payroll_rules_settings($selected_id) {
	...
	if($payroll_structure) {
		...
		if($pay_basis == MONTHLY_SALARY)
		    $th = array(trans("Payroll Rules"),trans("Monthly Earnings"),trans("Monthly Deductions"));
		if($pay_basis == DAILY_WAGE)
			$th = array(trans("Payroll Rules"),trans("Daily Earnings"),trans("Daily Deductions"));
		table_header($th);
        ...
        if($new)
            submit_center('submit', trans("Save salary structure"), true, '', 'default');
        else {
            submit_center_first('submit', trans('Update'), trans('Update salary structure data'), 'default');
            submit_center_last('delete', trans("Delete"), trans('Delete salary structure if have been never used'), true);
        }
		div_end();
    }
    else
		display_error(trans("Payroll rules not defined for this salary scale"));
}
...
if (db_has_salary_scale()) {
	...
	salaryscale_list_cells(trans("Salary scale: "), 'salary_scale_id', null, trans('Select salary scale'), true);
	...
	display_note(trans('Before you can run this function Salary Scales must be defined and add Payroll Rules to them.'));
}
