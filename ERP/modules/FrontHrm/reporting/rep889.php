...
function get_payslip_allowance($payslip_no) {
    ...
    return db_query($sql, trans('Could not get payslip details'));
}
...
$pdf->writeHTMLCell(2*$x/3-30, 0, 15, 40, trans('Company:').'&nbsp;'.$comp_name, 0, 0, 0, true);
$pdf->writeHTMLCell($x/3-30, 0, 2*$x/3+20, 45, trans('Date:').'&nbsp;'.Today(), 0, 0, 0, true);
$pdf->writeHTMLCell($x, 0, 15, 45, trans('Address:').'&nbsp;'.$comp_adrs, 0, 0, 0, true);
$pdf->writeHTMLCell($x, 0, 15, 50, trans('Phone:').'&nbsp;'.$comp_phone, 0, 0, 0, true);

$pdf->SetFont('dejavu', 'BI', 25);
$pdf->Write(0, trans('Payslip'), '', 0, 'C', true, 0, false, false, 0);
