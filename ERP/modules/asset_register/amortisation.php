...
page(trans($help_context = "Amortisation Schedule"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(trans("The valuation year cannot be empty."));
		set_focus('valuationyear');
	}
	if (strlen($_POST['assetvalue']) == 0) 
	{
		$input_error = 1;
		display_error(trans("The asset value cannot be empty."));
	...
	if ($input_error != 1) 
	{
    	...
    	else 
    	{
			...
			display_notification(trans('New Asset value has been added'));
    	}
...
if ($Mode == 'Delete')
{
	...
	display_notification(trans('Selected asset valuation has been deleted'));
...
$th = array(trans("Asset Type"),trans("Asset Name"),trans("Serial Number"), trans("Purchase Date"), trans("Purchase Price"), trans("Amortise"));
...
$th = array(trans("Valuation Year"),trans("Asset Value"),trans("Value Change"), "");
inactive_control_column($th);
table_header($th);
$k = 0;
while ($myrow = db_fetch($vresult)) 
{
	...
	amount_cell($myrow["value_change"]);
 	delete_button_cell("Delete".$myrow['asset_valuation_id'], trans("Delete"));
...
text_row(trans("Year of Valuation :"), 'valuationyear', null, 50, 50);
amount_row(trans("Asset Value :"), 'assetvalue', null, null, null, 2);
...
$th = array(trans("Year"),trans("Asset Value"),trans("Depreciation"));
inactive_control_column($th);
table_header($th);
$k = 0;
