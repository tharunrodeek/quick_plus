...
page(trans($help_context = "Assets"), false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(trans("The purchase value cannot be empty."));
		set_focus('rate');
	}
	...
	if ($input_error != 1) 
	{
    	if ($selected_id != -1) 
    	{
			...
			display_notification(trans('Selected asset type has been updated'));
    	} 
    	else 
    	{
			...
			display_notification(trans('New Asset type has been added'));
    	}
...
function can_delete($selected_id)
{
	...
	{
		display_error(trans("Cannot delete this asset because amortisation transactions have been created referring to it."));
		return false;
	}
...
$th = array(trans("Asset Type"),trans("Asset Name"),trans("Serial Number"), trans("Purchase Date"),trans("Purchase Value"), trans("Current Value"), "", "", trans("A"));
...
while ($myrow = db_fetch($result)) 
{
	...
	amount_cell($myrow["current_value"]);
 	...
 	edit_button_cell("Edit".$myrow['asset_id'], trans("Edit"));
	inactive_control_cell($myrow["asset_id"], $myrow["inactive"], 'assets', 'asset_id');
 	delete_button_cell("Delete".$myrow['asset_id'], trans("Delete"));
...
// $_POST['location'] = "";
// $_POST['condition'] = trans("Good");
// $_POST['acquisition'] = trans("Purchase");
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
...
asset_type_list_row(trans("Asset Type :"), 'type', null, true);
text_row(trans("Asset Name :"), 'name', null, 50, 50);
text_row(trans("Serial Number :"), 'serial', null, 50, 50);
text_row(trans("Tag Number :"), 'tag', null, 50, 50);
date_row(trans("Purchase Date :"), 'purchase_date', '', null, 0, 0, 0, null, true);
amount_row(trans("Purchase Value :"), 'purchase_value', null, null, null, 2);
text_row(trans("Location :"), 'location', null, 50, 50);
text_row(trans("Condition :"), 'condition', null, 50, 50);
text_row(trans("Acquisition :"), 'acquisition', null, 50, 50);

echo "<tr><td colspan='2'><hr/></td></tr>";

amount_row(trans("Disposal Amount :"), 'disposal_amount', null, null, null, 2);
date_row(trans("Disposal Date :"), 'disposal_date', '', null, 0, 0, 0, null, true);
