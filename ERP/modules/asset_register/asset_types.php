...
page(trans($help_context = "Asset Types"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(trans("The asset type name cannot be empty."));
		set_focus('name');
	}
	if (strlen($_POST['rate']) == 0) 
	{
		$input_error = 1;
		display_error(trans("The depreciation rate cannot be empty."));
		set_focus('rate');
	}
...
function can_delete($selected_id)
{
	...
	{
		display_error(trans("Cannot delete this asset type because assets transactions have been created referring to it."));
		return false;
	}
...
if ($Mode == 'Delete')
{
	if (can_delete($selected_id))
	{
		...
		display_notification(trans('Selected asset type has been deleted'));
	}
	$Mode = 'RESET';
}
...
$th = array(trans("Asset Type Name"),trans("Depreciation Rate"), "", "");
...
while ($myrow = db_fetch($result)) 
{
	...
	inactive_control_cell($myrow["asset_type_id"], $myrow["inactive"], 'asset_types', 'asset_type_id');
 	edit_button_cell("Edit".$myrow['asset_type_id'], trans("Edit"));
 	delete_button_cell("Delete".$myrow['asset_type_id'], trans("Delete"));
	end_row();
}
...
text_row(trans("Asset Type:"), 'name', null, 50, 50);
percent_row(trans("Depreciation Rate:"), 'rate', null, 50, 50);

gl_all_accounts_list_row(trans("Asset Account:"), 'asset_account');
gl_all_accounts_list_row(trans("Depreciation Account:"), 'depreciation_account');
gl_all_accounts_list_row(trans("Accumulated Depreciation:"), 'accumulated_account');
gl_all_accounts_list_row(trans("Asset Revaluation Account:"), 'valuation_account');
gl_all_accounts_list_row(trans("Disposal Account:"), 'disposal_account');
