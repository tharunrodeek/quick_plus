...
function display_type ($type, $typename, $yr, $mo, $convert, &$dec, &$rep, $dimension, $dimension2)
{
	...
	if ($printtitle)
	{
		...
		$rep->SetCellPadding(0);
		$fill = 0;
		$rep->TextCol(0, 1,	trans('Total') . " " . $typename, 0, 4, 0, $fill, NULL, 1);
		$rep->SetCellPadding($oldcMargin);
		for ($i = 1; $i <= 12; $i++)
...
function print_annual_balance_breakdown_detail()
{
	...
	if ($destination)
   	...
   	$title = trans("Annual Balance Breakdown - Detailed");
	...
	*/
	...
	while ($class = db_fetch($classresult))
	{
		...
		$rep->SetCellPadding(0);
		$fill = 0;
		$rep->TextCol(0, 1,	trans('Total') . " " . $class["class_name"], 0, 4, 0, $fill, NULL, 1);
		$rep->SetCellPadding($oldcMargin);
		for ($i = 1; $i <= 12; $i++)
		{
	...
	$oldcMargin = $rep->GetCellPadding();
	$rep->SetCellPadding(0);
	$rep->TextCol(0, 1,	trans('Net Assets (Liabilities)'), 0, 4, 0, $fill, NULL, 1);
	$rep->SetCellPadding($oldcMargin);
	for ($i = 1; $i <= 12; $i++)
	...
	$oldcMargin = $rep->GetCellPadding();
	$rep->SetCellPadding(0);
	$rep->TextCol(0, 1,	trans('Total Profit/Loss'), 0, 4, 0, $fill, NULL, 1);
	$rep->SetCellPadding($oldcMargin);
	for ($i = 1; $i <= 12; $i++)
	{
