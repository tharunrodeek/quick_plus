...
function list_payroll_taxes() {
    ...
    $th = array(
        ...
        trans("Default Rate"),
        trans("Threshold"),
        trans("Payroll GL Account"),
        trans("Clearinghouse GL Account"),
        "",
        ""
...
function display_main_payroll_tax_form($id) {
    ...
    text_row(trans("Description:"), 'name', $_POST['name'], 40, 80);

    payroll_combo_row(trans("Type:"), 'tax_type', $_POST['tax_type'], $payroll_tax_type_array, true); //tiered, adv percent, percent, fixed amount
    switch ($_POST['tax_type']) {
        case 1: //Tiered
            amount_row(trans("Annual Allowance:"), 'allowance', $_POST['allowance']);
            break;
        case 2: //Percentage with threshold
            small_amount_row(trans("Tax Rate:"), 'rate', '', "", "%", user_percent_dec()); //TODO what is user_percent_dec?
            amount_row(trans("Threshold:"), 'threshold', $_POST['threshold']);
            break;
    ...
    gl_all_accounts_list_row(trans("Payroll Accrual Account:"), 'payroll_gl_code', null); //which account are we posting to?
    gl_all_accounts_list_row(trans("Clearinghouse Account:"), 'clearinghouse_gl_code', null); //which account are we using to pay from? Money will be moved from accrual account to clearinghouse account when tax is paid.
...
function can_process() {
    ...
    if (strlen($_POST['name']) == 0) {
        display_error(trans("You have to call this tax something."));
        set_focus('name');
        return false;
    }
...
function handle_submit(&$selected_id) { //note: this is a pointer to $selected_id
    ...
    if ($selected_id == -1) { // a new record
        ...
        commit_transaction();
        display_notification(trans("A new tax type has been added."));
        ...
        display_notification(trans("The tax information has been updated."));
    }
}
...
page(trans($help_context = "Payroll Taxes")); //how do we make "help context"?
...
display_note(trans("To avoid problems with manual journal entry all tax types should have unique Payroll GL accounts."), 0, 1);
...
if ($selected_id == -1) {
    submit_center('submit', trans("Add New"), true, '', 'default');
} else {
    submit_center('submit', trans("Update"), trans('Update tax info'), @$_REQUEST['popup'] ? true : 'default');
}
