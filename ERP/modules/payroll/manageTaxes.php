...
page(trans($help_context = "Taxes/Deductions"));
...
if (get_post('addupdate')) {
    ...
    if ($_POST['code'] == '') {
        $input_error = 1;
        display_error( trans("Tax Code cannot be empty."));
        set_focus('code');
    } elseif ($_POST['name'] == '') {
        $input_error = 1;
        display_error( trans("Tax Name cannot be empty."));
        set_focus('name');
    }
    ...
    if ($input_error == 0) {
        if ($new_tax_type) {
            ...
            display_notification(trans("New Tax type has been added."));
        } else {
            ...
            display_notification(trans("The tax type has been updated."));
        }
        $new_tax_type = true;
        clear_data();
...
if (get_post('Deletexx')) {
    display_notification(trans("DEV: a row is deleted."));
}
//--------------------------------------------------------------------------------------------------
...
start_row();
payroll_tax_types_list_cells(trans("Tax Type:"). "&nbsp;", 'tax_type_id', null, true, true, check_value('show_inactive'));
...
text_row(trans("Tax code:"), 'code', null, 3, 5);
text_row(trans("Tax Name:"), 'name', null, 50, 52);
gl_all_accounts_list_row(trans("Accrual Account:"), 'accrual_gl_code', null);
gl_all_accounts_list_row(trans("Expense Account:"), 'expense_gl_code', null);
record_status_list_row(trans("Current status:"), 'inactive');

table_section(2);

payroll_tax_deduction_list_row(trans('Tax/Deduction:'), 'type', $_POST['type'], false);
payroll_tax_resp_list_row(trans('Tax Responsibility:'), 'responsibility', $_POST['responsibility'], false);
payroll_tax_base_list_row(trans('Tax Base:'), 'tax_base', $_POST['tax_base'], false);
payroll_tax_period_list_row(trans('Figures Given As:'), 'tax_period', $_POST['tax_period'], false);
...
if ($new_tax_type) {
    display_note(trans("Values for tax rates can be entered after Tax Type is saved."), 0, 1);
}
start_table(TABLESTYLE, "width=50%");
$th = array (trans('Variable'), trans('From'), trans('To'), trans('Fixed Tax Amount'), trans('Subtract for variable'), trans('Fixed Tax pct.'), trans('Variable Tax pct.'), '','');
table_header($th);
if (!$new_tax_type) {
//    $res = get_tax_table_data($tax_type_id);
    ...
    edit_button_cell("Edit".'xx', trans("Edit"));
    delete_button_cell("Delete".'xx', trans("Delete"));
    end_row();
...
if ($new_tax_type) {
    ...
    submit_center_last('addupdate', trans("Add new Tax Type"), '', 'default');
} else {
    submit_center_first('addupdate', trans("Save Tax Type"), '', 'default');
    submit('Update', trans("Update view"), true, '', null);
    submit('delete', trans("Delete This Tax Type"), true, '', true);
