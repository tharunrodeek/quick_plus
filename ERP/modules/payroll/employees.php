...
function can_process()
{
    if (strlen($_POST['EmpFirstName']) == 0){
        display_error(trans("Who is this? Surely they have a name?"));
    ...
    if (strlen($_POST['EmpAddress']) == 0){
        display_error(trans("What? You hired a homeless guy?"));
    ...
    if (get_post('EmpInactive') == 1){
        ...
        {
        display_error( trans("A valid release date must be entered for inactive employees."));
    ...
    if (strlen($_POST['EmpTaxId']) == 0){//a more universal check - do we have _something_ for the tax id? If this is used, comment out or remove the "else" below and the pregmatch above
        display_error(trans("Big Brother needs to see your identification."));
        set_focus('EmpTaxId');
        return false;
    }/* else {
...
function handle_submit(&$selected_id)
{
    ...
    if ($selected_id) 
    {
        ...
        display_notification(trans("Employee has been updated."));
    } 
    else 
...
function handle_submit_pay($emp_id) {
    ...
    if (!check_num('rate', 0.01)){
        display_error(trans("Oh dear. We really ought to pay this person <em>something</em>"));
        set_focus('rate');
    } else {
        if ($_POST['rate_id'] != -1) {//editing record
            ...
            display_notification(trans("Pay rate has been updated."));
        } else {//new pay rate record
            //display_notification("rate id is ".$_POST['rate_id']);
            add_employee_compensation($emp_id, $_POST['pay_type'], input_num('rate'));
            display_notification(trans("Pay rate has been added."));
        }
        $Ajax->activate('_page_body');
    }
}
...
function employee_settings($selected_id){
    ...
    text_row(trans("Employee First Name:"), 'EmpFirstName', $_POST['EmpFirstName'], 40, 80);
    text_row(trans("Employee Last Name:"), 'EmpLastName', $_POST['EmpLastName'], 40, 80);
    textarea_row(trans("Address:"), 'EmpAddress', $_POST['EmpAddress'], 35, 5);

    text_row(trans("Phone:"), 'EmpPhone', $_POST['EmpPhone'], 40, 80);
    email_row(trans("e-Mail:"), 'EmpEmail', $_POST['EmpEmail'], 40, 80);

    date_row(trans("Birth Date:"), 'EmpBirthDate');
    textarea_row(trans("General Notes:"), 'EmpNotes', null, 35, 5);

    
    table_section(2);

    table_section_title(trans("Employment Information"));

    text_row(trans("Tax ID:"), 'EmpTaxId', $_POST['EmpTaxId'], 11, 11);
    payroll_emp_role_list_row(trans("Role:"), 'EmpRole', $_POST['EmpRole']); //Officer/Employee...
    payroll_payfreq_list_row(trans("Pay Frequency:"), 'EmpFrequency', $_POST['EmpFrequency']);//weekly/biweekly/semi-monthly...
    payroll_filing_status_list_row(trans("Filing Status:"), 'EmpStatus', $_POST['EmpStatus']);//Single, Married...
    text_row(trans("Allowances:"), 'EmpAllowances', $_POST['EmpAllowances'], 2, 2);
    amount_row(trans("Extra Witholding:"), 'EmpExtraWH', $_POST['EmpExtraWH']);

    table_section_title(trans(" "));//just a spacer
    date_row(trans("Hire Date:"), 'EmpHireDate');

    if ($selected_id) {
        check_row('Inactive:', 'EmpInactive');
        date_row(trans("Release Date:"), 'EmpReleaseDate');
    }
    ...
    div_start('controls');
    if (!$selected_id)
        submit_center('submit', trans("Add New Employee"), true, '', 'default');
    else 
        submit_center('submit', trans("Update Employee"), trans('Update employee data'), @$_REQUEST['popup'] ? true : 'default');
...
function employee_comp($emp_id){
    ...
    $th = array(trans("Description"), trans("Rate"), "", "");
    table_header($th);
    ...
    while($myrow = db_fetch($result)){
        ...
        edit_button_cell("EditPay".$myrow["id"], trans("Edit Pay"));
        delete_button_cell("DeletePay".$myrow["id"], trans("Delete Pay"));
    ...
    payroll_pay_type_list_row(trans("Type:"), 'pay_type', $_POST['pay_type'], true);
    amount_row(trans("Rate:"), 'rate', $_POST['rate']);
    hidden('rate_id', $_POST['rate_id']);
    end_table(1);  

    if (find_submit('EditPay') == -1)
        submit_center('SubmitPay', trans("Add Compensation"), true, '', 'default');
    else
        submit_center('SubmitPay', trans("Update Compensation"), true, '', 'default');
...
page(trans($help_context = "Manage Employees"), @$_REQUEST['popup'], false, "", $js); //TODO look into help_context and 'popup'
...
if (db_has_employees()){
    ...
    start_row();
    employee_list_cells(trans("Select an employee: "), 'EmpId', null,
        trans('New employee'), true, check_value('show_inactive'));
...
tabbed_content_start('tabs', array(
    ...
    'comp' => array(trans('&Compensation'), $selected_id),
    'taxes' => array(trans('&Taxes & Deductions'), $selected_id) ));
