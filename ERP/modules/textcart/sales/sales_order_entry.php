...
if (isset($_GET['AddedID'])) {
	...
	display_notification_centered(sprintf(trans("Invoice # %d has been entered."), $invoice));
	...
	$row = db_fetch($result);
	if ($row !== false)
		submenu_print(trans("Print &Receipt"), $row['trans_type_from'], $row['trans_no_from']."-".$row['trans_type_from'], 'prtopt');
	...
	if ($row === false)
		submenu_option(trans("Entry &customer payment for this invoice"), "/sales/customer_payments.php?SInvoice=".$invoice);
  ...
  $textcart_mgr->tab_display($orderitems,$_SESSION['Items'],"display_order_in_tab");
	...
	if ($_SESSION['Items']->trans_no == 0) {
		...
		submit_center_last('CancelOrder', $cancelorder,
	   		trans('Cancels document entry or removes sales order when editing an old document'), true);
