...
page(trans("Notification"), @$_REQUEST['popup']);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM'){
	...
	if (strlen($_POST['description']) == 0) {
		$input_error = 1;
		display_error(trans("The Notification description cannot be empty."));
	...
	if(strtotime($_POST['BeginTime']) > strtotime($_POST['EndTime'])){
		$input_error = 1;
		display_error(trans("The Notification Begin time is Greater than end time"));
		set_focus('BeginTime');
	}
...
if ($Mode == 'Delete'){
	...
	if (key_in_foreign_table($selected_id, 'kv_empl_job', 'id'))	{
		$cancel_delete = 1;
		display_error(trans("Cannot delete this Notification because Employees have been created using this Notification."));
	} 
	if ($cancel_delete == 0) {
		Delete('kv_empl_notification', array('id' => $selected_id));
		display_notification(trans("Selected Notification has been deleted"));
...
$th = array(trans("ID"), trans("Description"), trans("Begin Time"), trans("End Time"), "", "");
...
$k = 0; 
 ...
 foreach($result as $myrow) {
	...
	label_cell($myrow["BeginTime"].' '.trans(""));
	label_cell($myrow["EndTime"].' '.trans(""));
