...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM'){
	...
	if ($input_error != 1)	{
    	if ($selected_id != -1)     	{
    		Update('kv_empl_designation_group', array('id' => $selected_id), array('description' => $_POST['description']));    		
			$note = trans("Selected Designation group has been updated");
    	}   else  	{
    		Insert('kv_empl_designation_group', array('description' => $_POST['description']));
			$note = trans("New Designation group has been added");
    	}
...
if ($Mode == 'Delete'){
	...
	if (key_in_foreign_table($selected_id, 'kv_empl_job', 'desig_group'))	{
		$cancel_delete = 1;
		display_error(trans("Cannot delete this Type because Picklist have been created using this Designation."));
	} 
	if ($cancel_delete == 0) {
...
$th = array(trans("ID"), trans("Description"), "", "");
...
text_row_ex(trans("Designation Group:"), 'description', 30);
end_table(1);
