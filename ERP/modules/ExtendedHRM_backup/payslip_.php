...
if(get_post('employee_id')){
	...
	//display_error(json_encode($Allowance));
	...
	$pf_amt_actual = $gross_4_LOP = $ctc_final = 0; 
 	...
 	$duration  = GetEmplAttendanceDuration($employee_id,null,null, (int)get_post('month'), $year,-1,-1, ($Empl_info_row['status'] > 1 ? sql2date($Empl_job_row['date_of_status_change']) : null), $hrmsetup['weekly_off']);
	...
	//display_error($payroll_process_end);
		...
		//hidden('allowed_cl',$durartion[0]['cl']);
		hidden('allowed_al',$duration[0]['al']);
		...
		label_row(trans("Total No of days:"), $duration[0]['days_count']);
