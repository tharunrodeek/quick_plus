...
function print_customer_balances()
{
    ...
    $folk = $_POST['PARAM_5'];
    $hide_trans = $_POST['PARAM_4'];
    $orientation = $_POST['PARAM_6'];
    $destination = $_POST['PARAM_7'];
    $show_pending = $_POST['PARAM_3'];
...
//        $show_pending=false;
    ...
    $headers = array(trans('Trans Type'), trans('#'), trans('Cust Ref'), trans('Date'), trans('Due Date'), trans('Debits'), trans('Credits'),
        trans('Allocated'), trans('Outstanding'));

    if ($show_balance)
        $headers[8] = trans('Balance');
    $aligns = array('left', 'left', 'left', 'left', 'left', 'right', 'right', 'right', 'right');


    if($show_pending) {
        array_push($cols,610);
        array_push($headers,trans('Pending Total'));
        array_push($aligns,'right');
    }
...
//        3 => array('text' => trans('Sales Areas'), 'from' => $sarea, 'to' => ''),
        4 => array('text' => trans('Sales Man'), 'from' => $salesfolk, 'to' => ''),
//        5 => array('text' => trans('Currency'), 'from' => $currency, 'to' => ''),
//        6 => array('text' => trans('Suppress Zeros'), 'from' => $nozeros, 'to' => '')
    );

    $rep = new FrontReport(trans('Customer Ledger'), "CustomerLedger", user_pagesize(), 9, 'L');
    if ($orientation == 'L')
...
ROUND(SUM((detail.quantity * detail.unit_price)
                    - (detail.discount_amount * detail.quantity)
                    + ((detail.govt_fee + detail.bank_service_charge + 
                    detail.unit_tax * detail.quantity+
						  		detail.bank_service_charge_vat) * detail.quantity)),2) As pending_total 
...
FROM " . TB_PREF . "debtors_master cust
    ...
    LEFT JOIN 0_debtor_trans dt ON dt.debtor_no=cust.debtor_no 
    
    LEFT JOIN 0_debtor_trans_details detail ON detail.debtor_trans_no=dt.trans_no 
	AND detail.debtor_trans_type=dt.type AND detail.debtor_trans_type=10 AND detail.transaction_id = ''
...
//    dd($sql);
    ...
    while ($myrow = db_fetch($result)) {
        ...
        if (!$only_rec && !$hide_trans) {
            $rep->TextCol(3, 4, trans("Open Balance"));
        ...
        if($show_pending)
            $rep->AmountCol(9, 10, $myrow['pending_total'],$dec);
