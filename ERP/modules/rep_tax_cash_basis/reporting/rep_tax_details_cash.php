...
function print_tax_details_cash()
{
	...
	$rep = new FrontReport(trans('Tax Details (Cash Basis)'), "TaxDetails", user_pagesize(), 9, "L");
	$dec = user_price_dec();
    ...
    if (!$net) {
        $grossAmount =  trans('Gross');
    } else { 
        $grossAmount =  trans('Net');
    }
	
    $headers = array(trans('Type'),	trans('Ref'), trans('#'),trans('Date'), trans('Payer/Payee'),
          trans('Tax Amount'), $grossAmount.' '.trans('Output/Input'));

    if (!$net)
        $grossAmount = trans('Gross Amounts');
    else
        $grossAmount = trans('Net Amounts');
		
    $params =   array( 	0 => $comments,
	    1 => array('text' => trans('Period'), 'from' => $from, 'to' => $to),
        2 => array('text' => trans('Output/input'), 'from' => $grossAmount, 'to' => ''));
	...
	if ($rows > 0)
	{
		...
		$rep->Font('bold');
		$rep->TextCol(4, 5, trans("Total Payable or Refund"));
...
function print_tax_details_cash_subtotals($rep, $lastOutput, $lastTaxType, $lastRate,  $subTotalTax, $subTotalGross) 
{
    ...
    if ($lastOutput == "Output") {
        $rep->TextCol(1, 3, trans("Charged on sales") . " (" . trans("Output Tax")."):");
    } else {
        $rep->TextCol(1, 3,trans("Paid on purchases") . " (" . trans("Input Tax")."):");
    }
