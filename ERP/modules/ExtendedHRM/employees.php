 ...
 /*
	...
	if ($input_error != 1){
		...
		$array_info =  array( 'empl_salutation' => $_POST['empl_salutation'], 'empl_firstname' => $_POST['empl_firstname'],			'empl_lastname' => $_POST['empl_lastname'], 
                ...
                'leave_update_permission'=>$_POST['leave_update_permission']);
...
if (isset($_POST['delete']) && strlen($_POST['delete']) > 1) {
	...
	if (key_in_foreign_table($empl_id, 'kv_empl_salary', 'empl_id')){		
		...
		if (file_exists($filename) && !is_dir($filename))
			unlink($filename);
		display_notification(trans($empl_id));
		$_POST['empl_id'] = '';
		clear_data(); 
		set_focus('empl_id');
		$new_item = true;
...
<input type="text" name="emp_prefix" style="width: 23%;" value="'.$_POST['emp_prefix'].'">
...
<input type=\"text\" name=\"date_of_birth\" id=\"datepicker-13\"  /> <a tabindex=\"-1\" alt=\"date_of_birth\" autocomplete=\"off\">	</a>
</td>
</tr>";
	...
	if (isset($_POST['empl_id']) && !$new_item){
		...
		//date_row(trans("Date of Birth") . ":", 'date_of_birth', null, null, 0,0,0,null, true);
        echo "<tr><td class=\"label\">".trans("Date of Birth")."</td><td>
<input type=\"text\" name=\"date_of_birth\" id=\"datepicker-16\" value=".date('d-m-Y',strtotime($_POST['date_of_birth']))." /><a tabindex=\"-1\" alt=\"date_of_birth\">	<img src=\"../../../themes/daxis/images/cal.gif\" style=\"vertical-align:middle;padding-bottom:4px;width:16px;height:16px;border:0;\" alt=\"Click Here to Pick up the date\"></a>
</td>
</tr>";
...
echo "</select>
     ...
     /*  echo "<tr><td class=\"label\">".trans("Date of Join:")."</td><td>
        ...
        echo "<tr><td class=\"label\">".trans("Date of Join")."</td><td>
<input type=\"text\" name=\"joining\" id=\"datepicker-14\"  /> <a tabindex=\"-1\" alt=\"date_of_join\" >	</a>
</td>
</tr>";
        ...
        table_section_title(trans("Pay Elementsss"));
...
$tab_cont_ar = array(
		...
		'job' => array(trans("Job"), (user_check_access('HR_EMPL_INFO') ? $empl_id : null) ),
...
function calculateSum(repl_txt)
{
    ...
    $(".amount").each(function(index, box) {
        ...
        totalsum= Number(repl_number)+Number(totalsum);
 ...
 $(document).on('click','#datepicker-15', function () {
     $('#datepicker-15').datepicker({ dateFormat: 'dd/mm/yy' });
     $("#datepicker-15").datepicker();
 });

 $(document).on('click','#datepicker-16', function () {
     $('#datepicker-16').datepicker({ dateFormat: 'dd/mm/yy' });
