...
$cheque = new FrontReport(trans("Cheque Printing"), "cheque.pdf", user_pagesize(), 10);
...
$cheque->TextWrap($col4, $cheque->row, 50, trans("DATE"));
...
$cheque->NewLine();
if ($show_currencies)
	$cheque->TextWrap($col0, $cheque->row, $width_col0, trans("Amount in ").$from_trans['curr_code']);
...
for ($i = 0; $i < 2; $i++)
{
	...
	while ($alloc_row = db_fetch($result))
	{
		...
		if ($alloc_row['Total'] != $alloc_row['amt'])
			$cheque->TextWrap($tcol += 55, $cheque->row, $width_normal, trans(" (Left to allocate ").
	...
	{
		$cheque->NewLine();
		$cheque->TextWrap($col0, $cheque->row, 200, trans("Unallocated amount: ").number_format2($amt_total - $totalallocated, $dec));
		$cheque->NewLine();
	}
	$cheque->lineHeight = 16;
}
