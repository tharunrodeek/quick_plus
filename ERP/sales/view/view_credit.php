...
page(trans($help_context = "View Credit Note"), true, false, "", $js);
...
display_heading("<font color=red>" . sprintf(trans("CREDIT NOTE #%d"), $trans_id). "</font>");
echo "<br>";
...
start_table(TABLESTYLE, "width='100%'");
start_row();
label_cells(trans("Ref"), $myrow["reference"], "class='tableheader2'");
label_cells(trans("Date"), sql2date($myrow["tran_date"]), "class='tableheader2'");
label_cells(trans("Currency"), $myrow["curr_code"], "class='tableheader2'");
end_row();
start_row();
...
if (db_num_rows($result) > 0)
{
	...
	display_note(trans("There are no line items on this credit note."), 1, 2);
...
if ($sub_total != 0)
	label_row(trans("Sub Total"), $display_sub_tot, "colspan=6 align=right",
		"nowrap align=right width='15%'");
if ($myrow["ov_freight"] != 0.0)
{
	$display_freight = price_format($myrow["ov_freight"]);
	label_row(trans("Shipping"), $display_freight, "colspan=6 align=right", "nowrap align=right");
}

$tax_items = get_trans_tax_details(ST_CUSTCREDIT, $trans_id);
display_customer_trans_tax_details($tax_items, 6);

label_row("<font color=red>" . trans("TOTAL CREDIT") . "</font",
	"<font color=red>$display_total</font>", "colspan=6 align=right", "nowrap align=right");
end_table(1);

$voided = is_voided_display(ST_CUSTCREDIT, $trans_id, trans("This credit note has been voided."));
