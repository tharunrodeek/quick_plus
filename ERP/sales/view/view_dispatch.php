...
page(trans($help_context = "View Sales Dispatch"), true, false, "", $js);
...
display_heading(sprintf(trans("DISPATCH NOTE #%d"),$trans_id));
...
end_row();
start_row();
label_cells(trans("Dispatch Date"), sql2date($myrow["tran_date"]), "class='tableheader2'", "nowrap");
...
if (db_num_rows($result) > 0)
{
	...
	display_note(trans("There are no line items on this dispatch."), 1, 2);
...
label_row(trans("TOTAL VALUE"), $display_total, "colspan=6 align=right",
	"nowrap align=right");
end_table(1);

is_voided_display(ST_CUSTDELIVERY, $trans_id, trans("This dispatch has been voided."));
