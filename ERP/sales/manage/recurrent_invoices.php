...
page(trans($help_context = "Recurrent Invoices"), false, false, "", $js);

check_db_has_template_orders(trans("There is no template order in database.
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if (!get_post('group_no'))
	{
		...
		if (get_post('debtor_no'))
			display_error(trans("This customer has no branches. Please define at least one branch for this customer first."));
		else
			display_error(trans("There are no tax groups defined in the system. At least one tax group is required before proceeding."));
		set_focus('debtor_no');
	}
	if (strlen($_POST['description']) == 0) 
	{
		$input_error = 1;
		display_error(trans("The invoice description cannot be empty."));
		set_focus('description');
	}
	if (!check_recurrent_invoice_description($_POST['description'], $selected_id))
	{
		$input_error = 1;
		display_error(trans("This recurrent invoice description is already in use."));
		set_focus('description');
	}
	...
	{
		$input_error = 1;
		display_error(trans("No recurence interval has been entered."));
		set_focus('days');
	}
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
			...
			$note = trans('Selected recurrent invoice has been updated');
    	} 
    	else 
    	{
			...
			$note = trans('New recurrent invoice has been added');
    	}
...
if ($Mode == 'Delete')
{
	...
	if ($cancel_delete == 0) 
	{
		...
		display_notification(trans('Selected recurrent invoice has been deleted'));
	} //end if Delete area
	$Mode = 'RESET';
} 
...
$th = array(trans("Description"), trans("Template No"),trans("Customer"),trans("Branch")."/".trans("Group"),trans("Days"),trans("Monthly"),trans("Begin"),trans("End"),trans("Last Created"),"", "");
table_header($th);
$k = 0;
...
templates_list_row(trans("Template:"), 'order_no');

customer_list_row(trans("Customer:"), 'debtor_no', null, " ", true);

if ($_POST['debtor_no'] > 0)
	customer_branches_list_row(trans("Branch:"), $_POST['debtor_no'], 'group_no', null, false);
else	
	sales_groups_list_row(trans("Sales Group:"), 'group_no', null);

small_amount_row(trans("Days:"), 'days', 0, null, null, 0);

small_amount_row(trans("Monthly:"), 'monthly', 0, null, null, 0);

date_row(trans("Begin:"), 'begin');

date_row(trans("End:"), 'end', null, null, 0, 0, 5);

if ($selected_id != -1 && @$_POST['last_sent'] != "")
	date_row(trans("Last Created"), 'last_sent');
