...
page(trans($help_context = "Sales Groups"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(trans("The sales group description cannot be empty."));
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
			...
			$note = trans('Selected sales group has been updated');
    	} 
    	else 
    	{
    		add_sales_group($_POST['description']);
			$note = trans('New sales group has been added');
    	}
...
if ($Mode == 'Delete')
{
	...
	{
		$cancel_delete = 1;
		display_error(trans("Cannot delete this group because customers have been created using this group."));
	} 
	if ($cancel_delete == 0) 
	{
		delete_sales_group($selected_id);
		display_notification(trans('Selected sales group has been deleted'));
	} //end if Delete group
	$Mode = 'RESET';
} 
...
$th = array(trans("ID"), trans("Group Name"), "", "");
...
text_row_ex(trans("Group Name:"), 'description', 30); 
