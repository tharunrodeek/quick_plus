...
if (isset($_GET['FixedAsset'])) {
	...
	$_POST['fixed_asset'] = 1;
	$_SESSION['page_title'] = trans($help_context = "Fixed Assets Movement");
} else {
	$_SESSION['page_title'] = trans($help_context = "Inventory Item Movement");
}
...
if (get_post('fixed_asset') == 1)
	check_db_has_fixed_assets(trans("There are no fixed asset defined in the system."));
else
...
{
	if (get_post('fixed_asset') == 1) {
		stock_items_list_cells(trans("Item:"), 'stock_id', $_POST['stock_id'],
		...
		stock_costable_items_list_cells(trans("Item:"), 'stock_id', $_POST['stock_id']);
}
...
locations_list_cells(trans("From Location:"), 'StockLocation', null, true, false, (get_post('fixed_asset') == 1));

date_cells(trans("From:"), 'AfterDate', '', null, -user_transaction_days());
date_cells(trans("To:"), 'BeforeDate');

submit_cells('ShowMoves',trans("Show Movements"),'',trans('Refresh Inquiry'), 'default');
end_row();
end_table();
end_form();
...
$th = array(trans("Type"), trans("#"), trans("Reference"));

if ($display_location)
	array_push($th, trans("Location"));

array_push($th, trans("Date"), trans("Detail"), trans("Quantity In"), trans("Quantity Out"), trans("Quantity On Hand"));
...
label_cell("<b>".trans("Quantity on hand before") . " " . $_POST['AfterDate']."</b>", "align=center colspan=$header_span");
...
start_row("class='inquirybg'");
label_cell("<b>".trans("Quantity on hand after") . " " . $_POST['BeforeDate']."</b>", "align=center colspan=$header_span");
