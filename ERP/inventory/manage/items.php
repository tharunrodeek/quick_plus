...
//        $_POST['subcategory_2'] = 0;
        ...
        if (get_post('NewStockID') != get_post('stock_id') || get_post('addupdate')) { // first item display
            ...
            $_POST['notify_customer'] = $myrow["notify_customer"];
            $_POST['expired_in_days'] = $myrow["expired_in_days"];
            $_POST['notify_before_days'] = $myrow["notify_before_days"];



            //SUB_CAT_FILTER
            $sql = "SELECT * FROM 0_subcategories WHERE id=".$myrow['sub_category_id'];
            $result = db_query($sql);
            $subcat_row = db_fetch($result);
            $_POST['subcategory_1'] = $subcat_row['parent_sub_cat_id'];
            $_POST['subcategory_2'] = $subcat_row['id'];

            if($subcat_row['parent_sub_cat_id'] == 0) {
                $_POST['subcategory_2'] = null;
                $_POST['subcategory_1'] = $subcat_row['id'];
            }
...
//        stock_item_types_list_row(trans("Item Type:"), 'mb_flag', null);
...
//		'movement' => array(trans('&Transactions'), (user_check_access('SA_ITEMSTRANSVIEW') ? $stock_id : null)),
